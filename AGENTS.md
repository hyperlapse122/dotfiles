...
External skills come from `agents.skills.external` -> `.chezmoiexternals/ai-agents.toml` -> `~/.agents/skills/<name>`; keep the exact isolated subtree and no pin-bump workflow. GitLab-sourced skills (glab, glab-stack) are raw single-file externals (`type = "file"` on `<name>/SKILL.md`) pinned to the glab release via `.chezmoitemplates/glab-release-ref.tmpl`, which `.chezmoiexternals/vcs.toml`'s glab binary also consumes. Locally-authored personal skills (unpublished, e.g. `daily-report`) are managed directly under `dot_agents/skills/<name>/` and deploy to `~/.agents/skills/<name>`. `dot_agents/skills/` is harness-neutral and is omp's sole managed skills source.
...
`agents.omp.settings.disabledProviders` is the provider availability gate, and it is load-bearing rather than cosmetic: openrouter and opencode-zen are token-metered aggregators whose keys stay provisioned, but the backends are OFF so no AUTOMATIC selection path can reach them. `retry.fallbackChains` was never that path — retry walks declared chains only — but omp's catalog-wide scans (built-in `priority.json` role chains with bare patterns, the text-only-model vision fallback, the smol/slow finders, `providers.imageOrder`'s built-in tail) rank by incidental catalog order, and those two providers served 473 of the 562 available ids on this host. `disabledProviders` applies before any credential check, so it is the only setting that removes a backend from every one of those paths; the cost is that a disabled provider also leaves `/model`. Consequence for model policy: a disabled provider disappears from `omp models --json`, which is the catalog the settings provisioner checks every declared selector and model-oriented chain key against, so no role, subagent override, or chain hop may name either provider while it is listed here.

Managed instruction targets are one file per harness, named for the native filename: `AGENTS.md` for omp (`dot_omp/private_agent/`). Deleting a managed source stops management but does not prune an already-deployed target; pruning is a separate explicit decision.
