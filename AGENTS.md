...
Compound-engineering is one versioned local archive owned by the neutral `agents.marketplaces` registry; omp consumes it. OMP consumes a dedicated bundled mxm4-haptic marketplace plugin from `~/.local/share/omp-plugins`; its self-contained extension imports the TypeScript client, uses OMP's pi-compatible event API, and depends on the cross-platform per-user daemon. Linux uses systemd, macOS uses launchd, and Windows uses a limited current-user Task Scheduler task. Eligible-host build, installation, manager, and readiness failures are fatal; runtime event-delivery failures stay silent and never block a turn.
