...
| `60-build/` | build on apply: the in-repo `crates/` + `packages/` trees (see above) and the externally fetched **Meridian pi/hermes scrub plugins** compiled from upstream source (Pi: latest release; Hermes: resolved `main` commit because it has no releases/tags); remote-only inputs use a rendered ref/commit as their onchange trigger rather than a source-tree fingerprint |
| `70-agents/` | agent provisioning, late so it lands on a fully provisioned host (see next section): dotagents MCPs, the compound-engineering plugin for Claude Code + Codex (`install-compound-engineering`), the local Claude Code plugin marketplace (`install-claude-plugins`), plus pi's packages (`install-pi-packages`) |
...
| `opencode-plugins-json.tmpl` | resolves the data-driven `.agents.opencode.plugins` list into exact npm/git specs for `opencode.json` and `tui.json`, including target-specific suffixes |
...
### compound-engineering — a native plugin in all three agents, not dotagents skills

`everyinc/compound-engineering-plugin` (the `ce-*` + `lfg` workflow skills) is installed through **each agent's own plugin mechanism**, because upstream ships a first-class plugin for every agent this repo runs. Each agent then loads the set natively — versioned, listable (`claude plugin list` / `codex plugin list`), removable in one command — instead of 29 loose skill directories vendored under `~/.agents/skills/`. dotagents still owns every *other* skill and all the MCP servers.

Two source files, one release:

- **Claude Code + Codex** — `.chezmoiscripts/70-agents/run_onchange_after_install-compound-engineering.sh.tmpl` (linux + darwin gate, container-KEPT like the dotagents install, since agent skills are wanted in a devbox too). It adds the upstream marketplace (`EveryInc/compound-engineering-plugin`, marketplace name `compound-engineering-plugin`) and installs the `compound-engineering` plugin in each CLI. Like `install-claude-plugins`, `claude plugin install` writes only `enabledPlugins` + `extraKnownMarketplaces` in `~/.claude/settings.json` and never its `hooks` key, so aoe's session hooks and the `mxm4-haptic` plugin's hooks are untouched.
- **OpenCode** — no plugin CLI; its plugins are declared in config, so it takes the same release as a git dependency in the `plugin` array of `dot_config/opencode/readonly_opencode.json.tmpl` (`compound-engineering@git+https://github.com/EveryInc/compound-engineering-plugin.git#<tag>`, the pinned form upstream documents in its `.opencode/INSTALL.md`; the plugin is NOT published to npm, hence the git spec rather than the `<name>@<version>` form the other three plugins there use). Pure file phase — no script.

**Pinning is per-agent, and only Claude cannot do it.** All three resolve the SAME tag through `.chezmoitemplates/compound-engineering-ref.tmpl` (newest release filtered by the `compound-engineering-` tag prefix — the repo interleaves `marketplace-*` / `cursor-marketplace-*` / `cli-*` trains, so a plain `gitHubLatestRelease` would eventually resolve to the wrong one), so they can never drift onto different releases. Codex pins the marketplace snapshot with `marketplace add --ref <tag>` and OpenCode pins with `#<tag>`; **`claude plugin marketplace add` takes no ref at all** and tracks the repo's default branch — an upstream CLI limitation, not a choice. The script's `claude plugin marketplace update` refreshes that clone, so Claude lands on `main` while the other two sit on the tag.

**onchange trigger:** the resolved tag is rendered into the script body and IS the trigger (there is no external file to fingerprint) — a new upstream release changes the script's content and re-runs the install. Every command is idempotent, with **one** exception the script handles explicitly: on a ref BUMP, `codex plugin marketplace add` refuses to move a configured pin in place (`already added from a different source`, exit 1), so the codex path removes the marketplace and re-adds it at the new tag. Soft-skip throughout — a missing CLI or a failed install warns and exits 0, and one agent failing never blocks the other (with the standard onchange trade-off: that clean exit-0 is recorded as done, so re-run by hand or with `chezmoi apply --force`).
...
- **All agent configuration**: `.chezmoidata/agents.yaml`. This is the single
  ...
  - `agents.opencode.plugins` is the single plugin manifest for both
    ...
    source, npm/git install form, version resolver, and target suffixes.
...
- **Standalone CLIs outside mise** come from `.chezmoiexternals/`, pinned to the
  ...
  and atomically promoting the new tree. The two `meridian-plugin-*-scrub`
...
- **Meridian** is the POSIX user-scoped Claude Max proxy. Its latest GitHub
  ...
  binary. Pi's readonly `models.json` provider override routes Anthropic to the
  loopback service with `x-meridian-agent: pi`, preserving Pi's built-in model
  catalog and its existing default model. OpenCode does not load Meridian or
  route providers through it.

  **Prompt scrubbers:** `rynfar/meridian-plugin-pi-scrub` tracks its newest
  `meridian-plugin-pi-scrub-v<semver>` GitHub release in the same
  `ai-agents.toml`; its archive URL is pinned to the release's 40-char target
  ...
  two sibling `run_onchange_after_build-meridian-plugin-*-scrub.sh.tmpl`
  scripts run locked `npm ci` + the explicit TypeScript build with mise-managed
  Node, validate plugin identity/scope, and atomically link `pi-scrub.js` and
  `hermes-scrub.js` into `~/.config/meridian/plugins/`. Pi is restricted to its
  matching adapter; Hermes MUST have no adapter restriction because its headerless
  ...
  promotion. Both are POSIX-only and container-kept.
...
- **Only the CLI dotfiles deploy in a container.** The `.chezmoiignore` container
  ...
  when mise is absent), the two Meridian scrub-plugin builds (the agent CLIs
