...
- **Ubuntu-specific scripts** (`linux-ubuntu` gate = `{{ if eq (printf "%s-%s" .chezmoi.os .chezmoi.osRelease.id) "linux-ubuntu" -}}`):
  ...
  - `run_after_ubuntu-plymouth-breeze.sh.tmpl` — points the `default.plymouth` update-alternatives link at the breeze theme + `update-initramfs -u` (Ubuntu's plymouth de-brand; Fedora's is baked in `install-system-config`); idempotent, config-override only
...
- **De-branded to upstream Breeze** (both Fedora and Ubuntu Studio; GRUB is left native by choice): each distro's shipped desktop branding is reverted to stock Plasma/Breeze across four surfaces, **config-override only — no branding package is purged**:
  ...
  - **Boot splash (Plymouth)**: Fedora selects the theme via `system/linux/etc/plymouth/plymouthd.conf` (`Theme=breeze`), and `install-system-config` bakes it into the initramfs with `dracut --force` (Fedora-only block, `lsinitrd`-guarded so it rebuilds only when the theme is not yet baked). Ubuntu owns plymouth via the `default.plymouth` update-alternatives link, handled by `run_after_ubuntu-plymouth-breeze.sh.tmpl` (`linux-ubuntu` gate; sets breeze + `update-initramfs -u`, idempotent on the alternative's current value). Both skip when the breeze plymouth theme is absent.
