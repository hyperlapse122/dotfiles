...
│                                    # mxm4-haptic: MX Master 4 HID++ haptic helper (Linux + macOS via hidapi).
...
**Inside Solaar rules, use the [`mxm4-haptic`](crates/mxm4-haptic/) client** — `Execute: [mxm4-haptic, "<WAVEFORM>"]`. The [`mxm4-haptic`](crates/mxm4-haptic/) crate builds **three** Rust binaries into `~/.local/bin/` from a single `cargo install` during bootstrap. It runs on **Linux + macOS**: the daemon reaches the device through the cross-platform [`hidapi`](https://crates.io/crates/hidapi) crate (Linux **hidraw** backend, macOS **IOKit** shared open), pinned `=2.6.5` with `default-features = false` + `["linux-static-hidraw", "macos-shared-device"]`. `hidapi` is the daemon's only dependency and the one justified exception to the crates "zero-dep" rule (there is no pure-`std` path to HID off Linux); the Linux build needs `libudev` headers (`systemd-devel`, on macOS the system IOKit frameworks suffice). Both the Linux and macOS bootstraps build and autostart the daemon (`systemd --user` on Linux, a launchd agent on macOS) — see the macOS note at the end of this section. Windows is unsupported.

- **`mxm4-hapticd`** — a `systemd --user` daemon ([`mxm4-hapticd.service`](home/.config/systemd/user/mxm4-hapticd.service)) that is the **sole owner** of the Bolt receiver's HID++ session. It discovers the MX Master 4 and its HAPTIC feature index **over `hidapi`** (enumerating by Bolt VID/PID `046D:C548` and the HID++ control interface — USB interface 2 on Linux, vendor usage page `0xFF00` on macOS where `interface_number` is `-1`) — enabling receiver `0x41` connection notifications, re-announcing to learn which slots are connected, then sending Root `GetFeature(0x19B0)` to each connected slot; the slot that answers with a feature index instead of `ERR_INVALID_FEATURE_INDEX` is the mouse. **No `solaar show`, no on-disk cache** — device state lives only in memory and is re-discovered on reconnect (it watches `0x41` for disconnect/reconnect, including a different receiver slot). A **single I/O-owner thread** holds the one `HidDevice` (that type is `Send` but not `Sync`, and macOS IOKit does not promise a second handle sees the same unsolicited reports Linux hidraw broadcasts to every fd): it polls `read_timeout()` to drain input reports and, between reads, serializes playback with debounce + per-pulse pacing, writing the HID++ Long play report (feature `0x19B0`, function 4, ~1-2 ms, no ack wait). A separate accept thread feeds it waveform ids over an `mpsc` channel.
- **`mxm4-haptic`** — the thin one-shot client spawned by Solaar rules; validates the waveform name and forwards it to the daemon over the AF_UNIX socket (`$XDG_RUNTIME_DIR/mxm4-haptic.sock` on Linux; on macOS, which has no `XDG_RUNTIME_DIR`, it falls back to `$TMPDIR/mxm4-haptic.sock`), then exits. No device access (does not link `hidapi`) and no fallback — the daemon is the only writer, so if it is down the pulse is simply skipped.
...
Multi-word names **require quoting**. Intensity scales with `haptic-level` (0-100, persists across reconnect; set in the Solaar GUI — the dotfiles bootstrap deliberately does NOT enforce a level since the helper's no-ack-wait path makes mid-range intensities reliably perceived). No Solaar rule currently consumes this API: an earlier Haptic-hold-detection rule in [`home/.config/solaar/rules.yaml`](home/.config/solaar/rules.yaml) pulsed a waveform from a `Later` callback when hold-mode was detected, but the thumb-button rules there have since been converted to directional mouse-gesture bindings (no haptic playback). The live consumers of the daemon are the [`@h82/mxm4-haptic`](packages/mxm4-haptic/) TS client and the [`@h82/opencode-mxm4-haptic`](packages/opencode-mxm4-haptic/) plugin.
