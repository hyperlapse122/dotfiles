...
- Provisioning runs via `.chezmoiscripts/agents/run_onchange_after_install-dotagents-skills.sh.tmpl` (linux + darwin gate): it `mkdir -p`s the agent config dirs (`~/.claude`, `~/.codex`) first, then runs `dotagents --user install` through `mise exec npm:@sentry/dotagents@latest`, soft-skips on missing mise/git or install failure, and is kept in containers. Being `run_onchange_` (not `run_after_`), it re-runs only when its rendered content changes — it embeds a sha256 fingerprint of `dot_agents/private_readonly_agents.toml.tmpl` (via `include`, so no `op` resolution), so install re-runs whenever that config changes (agent/skill/MCP/trust edit, or a remote-skill `@ref` bumped by the weekly workflow) rather than on every apply. Tradeoff: a plain re-apply no longer re-runs install, so newer remote-skill releases (settled by `minimum_release_age`) are pulled only when agents.toml changes or the weekly workflow bumps a ref. The pre-`mkdir` is the **first-apply fix**: `dotagents install` symlinks `skills` into each agent's config dir (`ensureSkillsSymlink`) without creating the parent, and on a fresh machine `~/.codex` exists (codex external) but `~/.claude` does not until Claude Code is first run — so the symlink `ENOENT`s, the whole install exits non-zero, and the soft-skip silently swallows it, leaving skills/MCPs unconfigured until a later apply. Pre-creating the dirs makes the initial apply provision fully.
