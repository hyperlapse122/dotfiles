...
- **Ubuntu-specific scripts** (`linux-ubuntu` gate = `{{ if eq (printf "%s-%s" .chezmoi.os .chezmoi.osRelease.id) "linux-ubuntu" -}}`):
  ...
  - `run_after_ubuntu-luks-passphrase-prompt.sh.tmpl` — guarantees the early-boot LUKS passphrase prompt is VISIBLE on an encrypted host, the case the plymouth teardown can't cover. The teardown only undoes a *manual* bgrt `--set`; when `bgrt` (or any spinner theme) is the distro's **automatic** default the prompt is still swallowed (Launchpad #1864586) and an encrypted disk never unlocks — surfaced once the generic HWE kernel becomes the default boot entry. This script writes an `/etc/default/grub.d/99-luks-visible-passphrase.cfg` drop-in that strips **only** the `splash` boot arg from `GRUB_CMDLINE_LINUX_DEFAULT` (`quiet` kept) so Plymouth doesn't grab the console and `cryptsetup` prompts as text, then runs `update-grub`. Strictly an encrypted-Ubuntu concern: it acts **only** when the running system has a `crypto_LUKS` device (removing the drop-in + regenerating grub if encryption is later gone), never edits `/etc/default/grub` or any non-owned file, and never touches the passphrase keyslot. Deliberately `run_after` (rerunnable), NOT `run_once`, so a run skipped for lack of sudo retries next apply; idempotent no-op once in place.
...
- **De-branded** (both Fedora and Ubuntu Studio; GRUB and Plymouth are left native by choice): each distro's shipped desktop branding is reverted to stock across three surfaces — the desktop to Plasma/Breeze — **config-override only, no branding package is purged (and none is added)**:
  ...
  - **Boot splash (Plymouth) → left native**: Plymouth is no longer customized on either distro. The former Fedora `plymouthd.conf` (bgrt + `DeviceScale=2`) and the Ubuntu bgrt `update-alternatives` swap were removed — a `bgrt`/spinner splash could swallow the early-boot LUKS passphrase prompt on Ubuntu (Launchpad #1864586). Fedora's removed `/etc/plymouth/plymouthd.conf` is cleaned up via a Fedora-gated `REMOVED_ETC_PATHS` entry (never touching a native/user file at that path on Ubuntu); Ubuntu's alternative is reverted to the distro default by `run_after_ubuntu-plymouth-teardown.sh.tmpl` (only when bgrt is the currently-selected manual pick). Each distro keeps whatever Plymouth theme it ships. On an encrypted Ubuntu host the theme can still swallow the LUKS prompt, so `run_after_ubuntu-luks-passphrase-prompt.sh.tmpl` additionally drops `splash` from the kernel cmdline (LUKS-gated) to keep the passphrase prompt visible — see the Ubuntu-specific scripts list above.
