...
- **Root-owned `/etc` state** — use the installer's `REMOVED_ETC_PATHS` list
  (see `install-system-config`), which is idempotent and self-documenting.
