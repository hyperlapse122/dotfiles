...
**`pi-mcp-extension` is an MCP *client*, and it is INERT without `~/.pi/agent/mcp.json`** — installing it buys nothing on its own. That config is `dot_pi/agent/private_readonly_mcp.json.tmpl` (private 0600 + readonly 0444 — its headers carry `onepasswordRead` secrets, never a literal; unlike auth.json this one is NOT live-written by pi, so it stays a managed target). It ranges over the **same `.chezmoidata/agents.yaml`** (`agents.mcp.servers`) the dotagents and opencode templates consume — `codegraph` (stdio), `context7`, `websearch` (both streamable-http) — so pi reaches parity with Claude Code / Codex from a single source: **add an MCP server once in the data file and all three agents pick it up** (the former "add to both or pi falls behind" multi-file edit is gone).

- **All three carry `"lifecycle": "eager"`, and that is load-bearing.** A `lazy` server is only started by the interactive `/mcp:start` command, so under the non-interactive `pi -p` runs the `harness-delegation` skill documents, a lazy server would expose **zero** tools — the delegation would look fine and be toolless. Eager is the only lifecycle that works headlessly.
...
- **pi packages + model defaults**: `.chezmoidata/pi.yaml`. `pi.packages` renders
  into the `PACKAGES` array of
  `.chezmoiscripts/70-agents/run_onchange_after_install-pi-packages.sh.tmpl`
  (looped through `pi install`); `pi.defaultProvider` /
  `pi.defaultModel` / `pi.defaultThinkingLevel` render into the argv of
  `.chezmoiscripts/70-agents/run_onchange_after_config-pi.sh.tmpl`, which
  read-merge-writes them into the live `~/.pi/agent/settings.json` (NOT a
  managed target — pi live-writes it; the script preserves every key pi owns
  and writes back at 0644). Both scripts' rendered values ARE their onchange
  trigger (no fingerprint block). Tune packages or defaults HERE, not in the
  scripts.
- **Agent MCP servers, dotagents remote skills, and pi auth providers**: `.chezmoidata/agents.yaml`.
  `agents.mcp.servers` is transport-neutral (`stdio` → `command`/`args`; `http` →
  `url` + `headers`) and ranged over by the THREE agent-config templates —
  ...
  `dot_config/opencode/readonly_opencode.json.tmpl` (opencode) — each mapping it
  into its own format (dotagents TOML; pi `transport`/`lifecycle: "eager"`;
  opencode `type`/command-array). Header values are `op://` item-REFERENCE
  strings, NOT resolved secrets — `.chezmoidata/*.yaml` is static data, so
  `onepasswordRead` cannot run there; each consumer template resolves the
  reference at render, keeping secrets out of the fingerprint (`install-dotagents-skills`
  hashes the raw agents.toml.tmpl + this file — the config-solaar dual-hash form).
  `agents.skills.remote` (optional `ref`: absent → `gitHubLatestRelease`, present
  → literal branch head like improve's `main`) is ranged over by the dotagents
  template and the `install-dotagents-skills` resolved-refs comment; the twelve
  local path skills stay literal in the template (pending removal). Add or remove
  an MCP server HERE and all three agents pick it up — the old "add to both or
  pi falls behind" multi-file edit is gone. The opencode template's four former
  `{file:./secrets/...}` references (two MCP headers, two provider-auth apiKeys)
  are now inline `onepasswordRead`, and `dot_config/opencode/exact_private_secrets/`
  is deleted. `agents.pi.auth.providers` (`name` / `type` / `key`, the last an
  `op://` ref resolved at render) is the SAME pattern for pi's static api_key
  providers — ranged over by `run_onchange_after_config-pi-auth.sh.tmpl`, which
  overlays them into the pi-LIVE-WRITTEN `~/.pi/agent/auth.json` (NOT a managed
  target, since OAuth providers refresh tokens there; see the pi section). Add or
  remove a static api_key provider HERE; OAuth providers (codex, …) are written
  by pi itself and never declared.
