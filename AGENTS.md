...
| `90-services` | CLIProxyAPI and native service reconciliation |
...
Gate grammar is `<fact>`, `!<fact>`, or `<fact>.<value>`; YAML values beginning `!` MUST be quoted. `facts-validate.tmpl` rejects unknown facts, missing probes, invalid comparisons, malformed desktop values, and undeclared emitted facts. Consumers use only `facts.tmpl`, `facts-sh.tmpl`, and `facts-gate.sh.tmpl`, never the cache. Every fact is render-time and fail-safe: false MUST skip rather than grant. `FORCE_NVIDIA`, `FORCE_INTEL`, and `FORCE_UBUNTU_STUDIO` are installer-only overrides, never facts or system-file gates; `INSTALL_SYSTEM_CONFIG_FORCE` layers only on the headless guard. `stat` is required before absolute `include`; `glob` does not traverse symlinks; never use template `output` for a fact probe. Runtime scripts call shared `fact_gate()` and MUST NOT duplicate gate logic. Shared script guards receive `(dict "ctx" . "name" ...)`, not a bare name. `host-facts` is read-only and always exits 0.

`system/linux/etc/**` mirrors `/etc`; `.chezmoidata/system.yaml` owns mode, gate, check, distro-scoped `removed:`, and reload behavior. Edit the manifest and tree, not installers. `install-system-10-files`, `20-host`, and `30-network` have independent onchange triggers. Reload only changed services; Bluetooth restarts only when Bluetooth files differ. Network changes can restart firewalld, resolved, NetworkManager, and Tailscale. GDM greeter fingerprint login MUST remain disabled while in-session fingerprint remains available. Polkit may add `pam_fprintd` only when the Debian `common-auth` conjunction is true; MUST NOT use `pam-auth-update` or a `common-auth` integration for fingerprint support, because it would re-enable greeter fingerprint authentication and break the GDM/Fedora polkit boundary. Sudo remains password-only. Fcitx5 is the KDE/GNOME input method; the GNOME autostart entry MUST NOT add `X-GNOME-Autostart-Phase`. TPM2 uses the shared dracut/systemd-cryptenroll backend and blank/missing credentials skip. Ubuntu Studio packages, audio group, and realtime limits are all gated on `ubuntuStudio`; `FORCE_UBUNTU_STUDIO` MUST NOT grant the privilege file. Plymouth remains distro-native; SDDM Breeze is gated on its installed theme.

## CLIProxyAPI

CLIProxyAPI is workstation-only loopback infrastructure, not an agent route. Release and panel assets are checksum-verified externals selected by `cli-proxy-api-ref.tmpl` and `cli-proxy-api-panel-ref.tmpl`; malformed, duplicate, missing, or unsupported selectors fail render. The reviewed v7.2.80 source config is 0444, binds only `127.0.0.1:8317`, has empty client keys, disabled remote management/plugins and panel auto-update, and no debug/pprof/file logging. Do not auto-merge upstream config or fetch GitHub at runtime.

The reconciler resolves only the Management secret, writes a 0400 runtime config, stages the pinned local panel asset, and uses no agent/provider route. Its launcher validates owner-controlled non-symlink 0700 auth/work dirs, rejects a workdir `.env`, validates runtime config and auth metadata on every start, uses `umask 077` and `env -i` with the fixed allowlist, changes into the workdir, requires `commercial-mode: true`, and always passes `-local-model`.

Provider auth is the one persistent lifecycle exception. The loopback Management UI/API MAY create, refresh, and delete provider-auth entries under the validated live auth directory. The reconciler, launcher, and smoke validators MUST NOT read, print, or delete provider credentials or credential contents; they validate only that each top-level entry is a non-empty regular user-owned 0600 file with exactly one hard link. Nested dirs, symlinks, empty files, widened modes, foreign owners, and hard links fail closed. Other config writes remain unsupported, plugin-resource remains 404, and the panel is 200 only when a staged asset is enabled.

`.ci/smoke-cli-proxy-api.sh` is the readiness authority: identify the supervisor PID and active executable, require exactly one listener on 8317, local `/healthz`, parser-level 400 for model+agent, `/v1/models` schema, management 401/authorized loopback behavior, provider-auth lifecycle, panel enabled/disabled behavior, immutable/locked state, metadata-safe auth, and final PID/listener identity. Native CI additionally proves empty-auth interaction 503 and disabled synthetic auth. Rollback is allowed only for a manifest-proven binary-only candidate whose restored binary passes the same probe; policy drift removes launchable links or leaves the service disabled. Retain old verified candidates; never prune auth state.
...
`dot_agents/private_readonly_agents.toml.tmpl` is user-scoped **Claude/Codex only**: it owns their MCP/trust configuration and the `~/.claude/skills` -> `~/.agents/skills` symlink path. It MUST NOT own OpenCode config (`~/.config/opencode/opencode.json` is managed separately), MUST NOT carry hooks, and MUST NOT overwrite hooks. Hooks belong in plugins; live Claude `settings.json` and aoe/settings hooks MUST be preserved. External skills come from `agents.skills.external` -> `.chezmoiexternals/ai-agents.toml` -> `~/.agents/skills/<name>`; keep the exact isolated subtree and no pin-bump workflow. Compound-engineering is one versioned local archive for OpenCode/Claude/Codex; Pi deliberately installs its native unpinned `git:` package. Claude plugins use the local marketplace; plugin hooks are silent/exit-0 and never hook `SubagentStop`. Pi settings are managed readonly, packages reconcile with `pi update --extensions`, auth is a 0600 read-merge-write preserving Pi OAuth, and models are a managed empty-provider baseline without a CLIProxyAPI route. OpenCode plugins use `<pkg>@file:<absolute-path>`, not a bare absolute path.
...
| `.chezmoidata/haptic.yaml` | daemon/notify environment and Claude/Pi event waveforms |
