...
| `70-agents/` | agent provisioning, late so it lands on a fully provisioned host (see next section): dotagents skills/MCPs, plus the local Claude Code plugin marketplace (`install-claude-plugins`) |
...
| `compound-engineering-ref.tmpl` | newest `compound-engineering-*` release tag (the plugin repo interleaves other tag trains, so plain `gitHubLatestRelease` is wrong); shared by the agents.toml template and the install script's onchange refs so they cannot drift |
