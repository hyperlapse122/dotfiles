...
- **Ubuntu-specific scripts** (`linux-ubuntu` gate = `{{ if eq (printf "%s-%s" .chezmoi.os .chezmoi.osRelease.id) "linux-ubuntu" -}}`):
  ...
  - `run_after_ubuntu-luks-tpm2.sh.tmpl` — prompts (chezmoi `promptString`, blank = skip) for the existing LUKS passphrase and drives the deployed `~/.local/bin/setup-luks-tpm2-unlock.sh` **non-interactively** (passphrase handed over via the `SETUP_LUKS_TPM2_PASSPHRASE` env var, base64-wrapped in the transient run-script for shell-safety only) to enroll every LUKS device for clevis TPM2 auto-unlock. On Ubuntu the backend also pins the TPM kernel driver (`ensure_tpm_initramfs_modules` → `/etc/initramfs-tools/modules`) alongside the `tss` user (`ensure_tss_initramfs_hook`) into the initramfs before rebuilding it — without the driver `/dev/tpmrm0` is absent at early boot, so the clevis unlock that succeeded on the running system fails at boot and drops to an initramfs shell with `/dev/mapper/... does not exist` (clevis issue #136). Deliberately `run_after` (rerunnable, idempotent) so a run skipped for a blank passphrase / absent TPM / missing sudo retries next apply; every precondition soft-skips `exit 0`. A blank passphrase — always returned by non-interactive/CI `promptString` — renders no passphrase and takes the skip path. The tool's default interactive path is unchanged; the env-passphrase mode is opt-in (see its header).
