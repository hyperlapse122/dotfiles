...
├── .github/                         # GitHub config; workflows/packages.yml = CI for packages/.
...
- **CI: [`.github/workflows/packages.yml`](.github/workflows/packages.yml)** builds/typechecks/tests the workspace on pushes to `main` and PRs touching `packages/**`. It installs the toolchain from `packages/mise.toml` via mise-action (caching the mise tool installs) and caches Yarn 4's global cache (`~/.yarn/berry/cache`), then runs `yarn install --immutable` + `yarn turbo run build typecheck test`. This is the only CI in the repo; it does NOT run dotbot or touch `$HOME`.
