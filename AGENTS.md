...
- **Ubuntu-specific scripts** (`linux-ubuntu` gate = `{{ if eq (printf "%s-%s" .chezmoi.os .chezmoi.osRelease.id) "linux-ubuntu" -}}`):
  ...
  - `run_once_after_teardown-ubuntu-plymouth-bgrt.sh.tmpl` — one-shot teardown that reverts the `default.plymouth` update-alternatives link back to Ubuntu's automatic (distro-default) choice + `update-initramfs -u`, undoing the removed bgrt splash customization (a `bgrt`/spinner splash can swallow the early-boot LUKS passphrase prompt, Launchpad #1864586, leaving an encrypted host unable to unlock). Guarded/idempotent; a no-op on hosts that never carried the bgrt override. Plymouth is otherwise left native on both distros.
...
- **De-branded** (both Fedora and Ubuntu Studio; GRUB and Plymouth are left native by choice): each distro's shipped desktop branding is reverted to stock across three surfaces — the desktop to Plasma/Breeze — **config-override only, no branding package is purged (and none is added)**:
  ...
  - **Boot splash (Plymouth) → left native**: Plymouth is no longer customized on either distro. The former Fedora `plymouthd.conf` (bgrt + `DeviceScale=2`) and the Ubuntu bgrt `update-alternatives` swap were removed — a `bgrt`/spinner splash could swallow the early-boot LUKS passphrase prompt on Ubuntu (Launchpad #1864586). Fedora's removed `/etc/plymouth/plymouthd.conf` is cleaned up via `REMOVED_ETC_PATHS`; Ubuntu's alternative is reverted to the distro default by the one-shot `run_once_after_teardown-ubuntu-plymouth-bgrt.sh.tmpl`. Each distro keeps whatever Plymouth theme it ships.
