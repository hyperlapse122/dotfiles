...
- **Ubuntu-specific scripts** (`linux-ubuntu` gate = `{{ if eq (printf "%s-%s" .chezmoi.os .chezmoi.osRelease.id) "linux-ubuntu" -}}`):
  ...
  - `run_after_ubuntu-luks-tpm2.sh.tmpl` — prompts (chezmoi `promptString`, blank = skip) for the existing LUKS passphrase and drives the deployed `~/.local/bin/setup-luks-tpm2-unlock.sh` **non-interactively** (passphrase handed over via the `SETUP_LUKS_TPM2_PASSPHRASE` env var, base64-wrapped in the transient run-script for shell-safety only) to enroll every LUKS device for clevis TPM2 auto-unlock. On Ubuntu the backend makes the initramfs able to actually open the root LUKS device before rebuilding it: it (1) forces the `cryptsetup` binary + cryptroot scripts into the initramfs (`ensure_cryptsetup_in_initramfs` → `CRYPTSETUP=y` in `/etc/cryptsetup-initramfs/conf-hook`) — `cryptsetup-initramfs` only *Recommends*-ships and its heuristic can decline to bundle `cryptsetup`, leaving an initramfs with no `crypt*` executable so the root device is never opened and boot drops to an `(initramfs)` shell; (2) pins the TPM kernel driver (`ensure_tpm_initramfs_modules` → `/etc/initramfs-tools/modules`) alongside the `tss` user (`ensure_tss_initramfs_hook`) — without the driver `/dev/tpmrm0` is absent at early boot, so the clevis unlock that succeeded on the running system fails at boot with `/dev/mapper/... does not exist` (clevis issue #136); and (3) ensures the root device has an `/etc/crypttab` entry (`backend_crypttab_opts` returns `luks`, a no-op merge on a normal installer entry) so cryptroot/clevis know which device to open. `cryptsetup-initramfs` is pinned explicitly in `packages.yaml` and preflighted (`hooks/cryptsetup`). Deliberately `run_after` (rerunnable, idempotent) so a run skipped for a blank passphrase / absent TPM / missing sudo retries next apply; every precondition soft-skips `exit 0`. A blank passphrase — always returned by non-interactive/CI `promptString` — renders no passphrase and takes the skip path (the template guards `.luksPassphrase` with `hasKey` so the render-dotfiles empty-config internals render doesn't error). The tool's default interactive path is unchanged; the env-passphrase mode is opt-in (see its header). The `render-dotfiles` Ubuntu `apply` job asserts the rendered tool carries this backend + `CRYPTSETUP=y` fix.
...
- **De-branded** (both Fedora and Ubuntu Studio; GRUB and Plymouth are left native by choice): each distro's shipped desktop branding is reverted to stock across three surfaces — the desktop to Plasma/Breeze — **config-override only, no branding package is purged (and none is added)**:
