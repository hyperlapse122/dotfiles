...
Multi-word names **require quoting**. Intensity scales with `haptic-level` (0-100, persists across reconnect; set in the Solaar GUI — the dotfiles bootstrap deliberately does NOT enforce a level since the helper's no-ack-wait path makes mid-range intensities reliably perceived). No Solaar rule currently consumes this API: an earlier Haptic-hold-detection rule in [`home/.config/solaar/rules.yaml`](home/.config/solaar/rules.yaml) pulsed a waveform from a `Later` callback when hold-mode was detected, but the Overview rules there have since been simplified to plain toggles. The live consumers of the daemon are the [`@h82/mxm4-haptic`](packages/mxm4-haptic/) TS client and the [`@h82/opencode-mxm4-haptic`](packages/opencode-mxm4-haptic/) plugin.
