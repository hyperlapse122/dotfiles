...
| `resolve-op-refs-json.tmpl` | recursively emits maps/slices/scalars as JSON, resolving only string values beginning exactly `op://` through `onepasswordRead`; validated exact + reusable regex order rules preserve established non-lexical object order |
...
- **All agent configuration**: `.chezmoidata/agents.yaml`. This is the single
  ...
  - `agents.pi.packages` renders into the pi package installer's `PACKAGES`
    array. `agents.pi.defaults.{provider,model,thinkingLevel}` renders into the
    read-merge-write settings script. `agents.pi.auth.providers` supplies only
  ...
  - Static data stores secrets as `op://` reference strings. The recursive
    ...
    branches. `agents.opencode.providerKeyOrder` carries only the non-lexical map
    order needed to keep the rendered provider JSON byte-identical: exact rules
    require a complete key-set match, while reusable regex rules cover repeated
    model/cost/thinking shapes. Duplicate, stale, unmatched, or overlapping
    rules abort the render; omitted maps sort lexically.
