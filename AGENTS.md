...
- It declares three remote skills, each **`@`-pinned to an explicit ref** (a release tag, or a commit sha for repos that publish no releases — `shadcn/improve`) — `playwright-cli` (`microsoft/playwright-cli@<tag>`), `improve` (`shadcn/improve@<sha>`), `dotagents` (`getsentry/dotagents@<tag>`) — plus the **eleven** local skills under `dot_agents/skills/` as `path:skills/<name>` entries: `ci-cd-monitoring`, `daily-report`, `git-branch-cleanup`, `git-workflow`, `gitlab-issues`, `glab`, `glab-stack`, `js-package-managers`, `pr-mr`, `ship-issue`, `src-layout` (`glab` and `glab-stack` are currently empty placeholders — a `.gitkeep` only, no `SKILL.md` yet). `daily-report`, `git-branch-cleanup`, and `ship-issue` were formerly opencode slash-commands under `dot_config/opencode/commands/`; as skills they are read by Claude Code + Codex (via dotagents) and auto-discovered natively by opencode from `~/.agents/skills/<name>/SKILL.md`, so `agents` deliberately stays `["claude", "codex"]` — adding `opencode` would make dotagents write MCP config into the repo-managed readonly `~/.config/opencode/opencode.json`. The now-orphaned deployed `~/.config/opencode/commands` directory is cleaned up by the root `.chezmoiremove`. The same file also registers the MCP servers (`codegraph`, `context7`, Exa `websearch`, and the Z.ai servers), with secrets injected via `onepasswordRead`.
