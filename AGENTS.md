...
This is the chezmoi **source state** for `github.com/hyperlapse122/dotfiles`. Edit this checkout, never deployed `$HOME`; apply only when the user asks for deployment. The common guardrails in `.chezmoitemplates/agents-instructions.tmpl` (composed per harness into each agent wrapper) are binding; this file is the repository supplement and may add stricter local rules. The repository's own checked-in `CLAUDE.md` files — the root pair and `packages/` — must remain exactly `@AGENTS.md`; that sibling-mirror rule scopes to repository documentation only and never to chezmoi-managed instruction sources or their deployed targets (see *Agent surfaces and ownership*).
...
| `70-agents` | dotagents, Claude/Codex plugins, Claude values, and omp updates |
| `80-keys` | one-time GPG and age imports |
| `90-services` | user-service runtimes (CLIProxyAPI quadlet stack: data-root seeding, credential injection, rollout) |
...
All agent data is in `.chezmoidata/agents.yaml`. The managed harness set is Claude, Codex, and omp. MCP declarations are transport-neutral and feed the shared Claude/Codex dotagents target plus omp. `op://` strings resolve only at render. OAuth entries stay headerless for native stores. Figma authorization is on demand through `figma-auth omp`; never commit tokens.

`dot_agents/private_readonly_agents.toml.tmpl` is user-scoped **Claude/Codex only**. It owns their MCP/trust configuration and the `~/.claude/skills` -> `~/.agents/skills` symlink path. It MUST NOT carry or overwrite hooks. Hooks belong in plugins. Live Claude `settings.json` and aoe/settings hooks MUST be preserved. External skills come from `agents.skills.external` -> `.chezmoiexternals/ai-agents.toml` -> `~/.agents/skills/<name>`; keep the exact isolated subtree and no pin-bump workflow. GitLab-sourced skills (glab, glab-stack) are raw single-file externals (`type = "file"` on `<name>/SKILL.md`) pinned to the glab release via `.chezmoitemplates/glab-release-ref.tmpl`, which `.chezmoiexternals/vcs.toml`'s glab binary also consumes. Locally-authored personal skills (unpublished, e.g. `daily-report`) are managed directly under `dot_agents/skills/<name>/` and deploy to the same `~/.agents/skills/<name>` path; dotagents stays MCP-only and does not manage them.

Compound-engineering is one versioned local archive owned by the neutral `agents.marketplaces` registry. Claude, Codex, and omp consume that same archive. The mxm4-haptic marketplace plugin ships to Claude and Codex. OMP consumes a dedicated bundled marketplace plugin from `~/.local/share/omp-plugins`; its self-contained extension imports the TypeScript client, uses OMP's pi-compatible event API, and depends on the cross-platform per-user daemon. Linux uses systemd, macOS uses launchd, and Windows uses a limited current-user Task Scheduler task. Eligible-host build, installation, manager, and readiness failures are fatal; runtime event-delivery failures stay silent and never block a turn. Codex leaves `SubagentStop` unhooked and has no failed waveform because its Stop payload carries no error field. Claude/Codex plugin hooks merge with the aoe-managed settings hooks.

The mxm4-haptic phase-60 provisioners are `run_after_build-mxm4-haptic.{sh,ps1}.tmpl`, not onchange scripts. Component fingerprints and stamps keep plugin and daemon builds conditional. The every-apply run repairs live systemd, launchd, or Task Scheduler drift without rebuilding unchanged artifacts, then verifies the exact managed endpoint between two running-state checks. Keep the provisioning fixtures transactional and isolated; they MUST prove unchanged convergence, selective rebuilds, manager drift repair, and fatal build/install/manager/readiness boundaries.

An agent config file moves to per-key assertion when its vendor writes it during normal use and stays managed-readonly otherwise. omp's `config.yml` is on the assertion side. It is NOT a chezmoi target because omp rewrites it through `/model`, `/settings`, and first-run setup. `run_after_config-omp-settings.{sh,ps1}.tmpl` asserts each declared `agents.omp.settings` path into it on every apply and leaves every undeclared path to omp. A declared record-typed path is owned wholesale because `omp config set` replaces a record and rejects a path inside one. Declared omp settings MUST stay credential-free because that CLI receives values as process arguments. Credentials are declared under `agents.omp.auth.env` and reconciled as a closed set into its private `.env`; OAuth stays in `agent.db`. Retiring the old config target deliberately left the deployed file in place, so it has no `.chezmoiremove` line or `remove_` source. omp's `models.yml`, MCP file, and instructions stay managed-readonly. `~/.omp/agent/mcp.json` remains managed-readonly as a dated exception (2026-07-29) until per-key assertion covers a list-shaped target. OMP's auth, settings, compound-engineering, and haptic plugin provisioners all deploy on Windows; the haptic runtime remains excluded only in real containers.
...
| `.chezmoidata/haptic.yaml` | daemon/notify environment and Claude/Codex/omp event waveforms |
| `.chezmoidata/agents.yaml` | MCPs, skills, neutral marketplaces, Claude/Codex plugins, Claude values, and omp settings/auth/provider data |
...
User services that run as containers use Podman quadlets under `dot_config/containers/systemd/` — the CLIProxyAPI stack (proxy + CPA-Manager-Plus panel) is the first convention: one `<name>.network` quadlet shared by the stack's containers so they reach each other by container name, image refs pinned tag+digest through the release lock's `ociImage` entries, loopback-only `PublishPort`, per-container `:Z`-labeled bind mounts under a single data root (never anonymous volumes), and `WantedBy=default.target` so lingering starts them at login. Runtime seeding and credential injection belong to the `90-services` provisioner, never to the quadlet itself; containers are skipped by gating only the `.config/containers/systemd` subpath in the container-fact `.chezmoiignore` block (`containers/` also holds `auth.json` and `registries.conf`, which deploy everywhere). A laptop hibernation stop/restart pairing lives in a `system-sleep` hook (`system/linux/etc/systemd/system-sleep/cli-proxy-api.sh`), not in the quadlets.
