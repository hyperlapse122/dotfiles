...
- **Native look kept**: Ubuntu Studio's native branding (Plymouth splash, SDDM greeter, GRUB, wallpaper) is kept — the former branding scripts were deleted, not adapted. Global-theme control is split per-distro so each has exactly one owning script: `config-kde-darkmode` is **Fedora-only** and pins `LookAndFeelPackage=org.kde.breezedark.desktop` + `AutomaticLookAndFeel=false` (Breeze Dark, no day/night switching); `config-kde-autotheme` is **Ubuntu-Studio-only** (`linux-ubuntu` gate) and moves Ubuntu Studio off its custom Global Theme onto Plasma's default Breeze with `AutomaticLookAndFeel=true` + `DefaultLightLookAndFeel=org.kde.breeze.desktop` / `DefaultDarkLookAndFeel=org.kde.breezedark.desktop` (automatic Breeze Dark ⇄ Light, timed off the Night Light schedule). On Plasma < 6.5 the auto-switch keys are silently ignored, so `config-kde-autotheme` degrades to the `LookAndFeelPackage` Breeze Dark pin. Either way the Plasma session's icon theme, cursor, splash, and default wallpaper follow the active Breeze variant on next login — this cascades beyond just colors.
