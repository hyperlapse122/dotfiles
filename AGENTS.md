...
dependency changes the hash and re-triggers. Establish `$sourceDir` at the top
and range over the tracked sources (single files or globs):

```
{{ $sourceDir := .chezmoi.sourceDir -}}
...
{{ range (glob (joinPath $sourceDir "path/to/dep")) -}}
{{ if not (stat .).isDir -}}
...
`include . | sha256sum` hashes file CONTENT (a bare `sha256sum .` would hash the
path STRING); the `(stat .).isDir` guard skips the directories a `**` glob
matches. See `run_onchange_after_install-system-config`,
...
- **No dependency file? Onchange still applies** — the trigger is simply the
  ...
  `ubuntu-plymouth-teardown`, `ubuntu-tailscale-ufw`).
...
- **Ubuntu-specific scripts** (`linux-ubuntu` gate = `{{ if eq (printf "%s-%s" .chezmoi.os .chezmoi.osRelease.id) "linux-ubuntu" -}}`):
  ...
  - `run_onchange_after_ubuntu-plymouth-teardown.sh.tmpl` — reverts the removed bgrt splash customization: undoes the old manual `--set` of the `default.plymouth` update-alternatives link by running `update-alternatives --auto` + `update-initramfs -u -k all` (a `bgrt`/spinner splash can swallow the early-boot LUKS passphrase prompt, Launchpad #1864586, leaving an encrypted host unable to unlock). `run_onchange` with no external dependency (its trigger is its own content), and still NOT `run_once` — a `run_once` skip could never re-fire at all. Note the onchange trade-off: a run skipped for lack of sudo is recorded as done, so on such a host complete the revert with `chezmoi apply --force` (or by hand) rather than expecting an automatic retry. Strictly scoped to OUR change: it acts only when `bgrt` is the currently-selected theme in **manual** mode, never `--remove`s an alternative, and never resets the distro's automatic default or a user's own manual pick. Plymouth is otherwise left native on both distros.
