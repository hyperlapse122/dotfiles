...
- `dot_agents/private_readonly_agents.toml.tmpl` (private 0600 + readonly 0444, templated) renders to `~/.agents/agents.toml` and declares the MCP servers plus the twelve local `path:` skills. It sets `agents = ["claude", "codex"]` and a `[trust]` allowlist (`git_domains = [git.jpi.app]`; the former `github_orgs = [getsentry, microsoft, shadcn]` existed only for the three now-externalized remote skills and is gone — every remaining skill is a local `path:` source, which is always trusted).
...
- **All four carry `"lifecycle": "eager"`, and that is load-bearing.** A `lazy` server is only started by the interactive `/mcp:start` command, so under the non-interactive `pi -p` runs the `harness-delegation` skill documents, a lazy server would expose **zero** tools — the delegation would look fine and be toolless. Eager is the only lifecycle that works headlessly.
...
- **~/src project registry (garden)**: `src/encrypted_readonly_garden.yaml.age` →
  ...
  **age-encrypted** — this repo is public and the trees list (internal project
  ...
  agent-facing procedure is the `src-layout` skill under `dot_agents/skills/`).
- **All agent configuration**: `.chezmoidata/agents.yaml`. This is the single
  ...
  - `agents.skills.external` lists the agent skills fetched as chezmoi externals
    ...
    `ref` → a branch head resolved to a commit via `git ls-remote`). The twelve
    local skills stay literal `path:skills/<name>` entries in `agents.toml`
    because they have one consumer each.
