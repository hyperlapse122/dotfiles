...
`dot_agents/private_readonly_agents.toml.tmpl` is user-scoped **Claude/Codex only**: it owns their MCP/trust configuration and the `~/.claude/skills` -> `~/.agents/skills` symlink path. It MUST NOT own OpenCode config (`~/.config/opencode/opencode.json` is managed separately), MUST NOT carry hooks, and MUST NOT overwrite hooks. Hooks belong in plugins; live Claude `settings.json` and aoe/settings hooks MUST be preserved. External skills come from `agents.skills.external` -> `.chezmoiexternals/ai-agents.toml` -> `~/.agents/skills/<name>`; keep the exact isolated subtree and no pin-bump workflow. GitLab-sourced skills (glab, glab-stack) are raw single-file externals (`type = "file"` on `<name>/SKILL.md`) pinned to the glab release via `.chezmoitemplates/glab-release-ref.tmpl`, which `.chezmoiexternals/vcs.toml`'s glab binary also consumes, so the skills and the CLI never diverge by configuration. Locally-authored personal skills (unpublished, e.g. `daily-report`) are managed directly as chezmoi source under `dot_agents/skills/<name>/` and deploy to the same `~/.agents/skills/<name>` — dotagents stays MCP-only and does not manage them. Compound-engineering is one versioned local archive for OpenCode/Claude/Codex/AGY/omp; AGY installs its native bundle root, omp installs its marketplace entry, and Pi deliberately installs its native unpinned `git:` package. The mxm4-haptic plugin ships to Claude, Codex, AND AGY from local marketplaces (Claude/Codex carry a `hooks/hooks.json`; AGY carries a ROOT `hooks.json` wired to bundled helper scripts, because `agy plugin validate` only recognizes a root hooks file, not a `hooks/` dir); its hook wrappers are silent/exit-0 no-ops that never block or slow a turn — Codex leaves `SubagentStop` unhooked (root-only gating), AGY gates its Stop buzz on `fullyIdle`, and the AGY `ask_question` PreToolUse hook ALWAYS returns `{"decision":"allow"}` so it can never deny a tool. Codex has no failed waveform (its Stop payload carries no error field); its plugin hooks merge with the aoe-managed `~/.codex/hooks.json`. Pi settings are managed readonly, packages reconcile with `pi update --extensions`, auth is a 0600 read-merge-write preserving Pi OAuth, and models are a managed empty-provider baseline. An agent config file moves to per-key assertion when its vendor writes it during normal use and stays managed-readonly otherwise; omp's `config.yml` is the one on the assertion side. It is NOT a chezmoi target, because omp rewrites it through `/model`, `/settings`, and first-run setup, so `run_after_config-omp-settings.{sh,ps1}.tmpl` asserts each declared `agents.omp.settings` PATH into it on every apply and leaves every path beside that set to omp; a declared record-typed path is owned WHOLESALE, since `omp config set` replaces a record and rejects a path inside one, so an omp-written member inside `modelRoles` or `task.agentModelOverrides` does not survive an apply. Declared omp settings MUST stay credential-free because that CLI receives values as process arguments — credentials are declared under `agents.omp.auth.env`, reconciled as a closed set into its private `.env`, while OAuth stays in `agent.db`. Retiring the old target deliberately left the deployed file in place and nothing re-locks it, so it has no `.chezmoiremove` line and no `remove_` source. omp's `models.yml`, MCP, and instructions stay managed-readonly, and `~/.omp/agent/mcp.json` plus `~/.omo/omo.jsonc` remain managed-readonly as a dated exception (2026-07-29), not as a case the rule leaves open: omp rewrites its MCP list at runtime, so the rule already selects `mcp.json` for assertion, and it stays managed-readonly only until per-key assertion covers a list-shaped target. Both omp provisioners deploy on Windows while the haptic extension does not. OpenCode plugins use `<pkg>@file:<absolute-path>`, not a bare absolute path.
...
Managed instruction targets are one file per harness, named for whatever that harness reads natively: `CLAUDE.md` for Claude (`dot_claude/`), `GEMINI.md` for AGY (`dot_gemini/`), and `AGENTS.md` for Codex, OpenCode, Pi, and omp (`dot_codex/`, `dot_config/opencode/`, `dot_pi/private_agent/`, `dot_omp/private_agent/`). The sibling-`CLAUDE.md` mirror rule does NOT apply to managed files: a managed `AGENTS.md` source MUST NOT gain a `CLAUDE.md` sibling, and a missing one MUST NOT be re-added as drift repair. Removing such a mirror also needs a `.chezmoiremove` prune line, because deleting the source stops managing the target without deleting the already-deployed copy.
