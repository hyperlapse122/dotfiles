...
| `.chezmoiexternals/*` | external fetches, grouped by DOMAIN into six files (not one file per tool): `ai-agents.toml` (claude-code, codex, agy, opencode, pi, meridian, the pi/hermes Meridian scrub plugins, codegraph, aoe), `dev-tools.toml` (ast-grep, buf, marksman, shellcheck, wasm-pack, rust-analyzer, uv/uvx), `vcs.toml` (gh, glab, garden), `k8s.toml` (kubectl, helm, minikube), `system.toml` (docker credential helpers, wakatime-cli, virtio-win, prezto), `fonts.toml`. Everything but prezto/fonts and the source-built Meridian scrub plugins is a pinned standalone CLI binary or versioned CLI tree linked into `~/.local/bin`; the scrubbers are versioned/commit-pinned source trees consumed by `60-build/`. Add a new tool to the matching domain file; don't spawn a new one |
...
- `dot_agents/private_readonly_agents.toml.tmpl` (private 0600 + readonly 0444, templated) renders to `~/.agents/agents.toml` and is the single source of truth. It sets `agents = ["claude", "codex"]` and a `[trust]` allowlist (`github_orgs = [getsentry, microsoft, shadcn]`, `git_domains = [git.jpi.app]`).
...
- **Remote skills track their newest upstream release, resolved at render time — there are no hand-maintained pins.** Each entry in `.chezmoidata/agents.yaml` (`agents.skills.remote`) carries an optional `ref`: `playwright-cli` (`microsoft/playwright-cli`) and `dotagents` (`getsentry/dotagents`) omit it, so the template resolves `(gitHubLatestRelease "<repo>").TagName`. `improve` (`shadcn/improve`) publishes NO releases and NO tags, so its entry sets `ref: main` literally — a branch head, not a frozen sha. Because a ref is resolved fresh on every source-state read, **`minimum_release_age` is deliberately absent**: a settle window would reject a just-published release and fail the whole apply. These are GitHub API calls, so run chezmoi through the zsh wrapper (`zsh -ic 'chezmoi …'`) to get `GITHUB_TOKEN` injected — see Verify edits. Alongside them are the **twelve** local skills under `dot_agents/skills/` as `path:skills/<name>` entries: `ci-cd-monitoring`, `daily-report`, `git-branch-cleanup`, `git-workflow`, `gitlab-issues`, `glab`, `glab-stack`, `harness-delegation`, `js-package-managers`, `pr-mr`, `ship-issue`, `src-layout` (`glab` and `glab-stack` are currently empty placeholders — a `.gitkeep` only, no `SKILL.md` yet). `harness-delegation` is the cross-harness one: how to shell out non-interactively to the OTHER three agent CLIs this repo installs (`pi`, `codex`, `agy`) plus `opencode` — every fact in it was verified against the live CLIs, because the invocations are not guessable (`agy`'s `-p` takes the prompt as its flag *value*, `codex exec` has no `--full-auto`, `pi`'s built-in `google` provider has no credentials here; the configured pi defaults come from the settings-shaped `agents.pi.defaults` object, while `config-pi-auth` merges the static `kimi-coding` + `zai` entries, and `opencode run` ignores `opencode.json`'s top-level `model` because oh-my-openagent maps one per agent from `.chezmoidata/agents.yaml` (`agents.opencode.ohMyOpenagent`)). Re-verify against `--help` before editing it, never from memory. `daily-report`, `git-branch-cleanup`, and `ship-issue` were formerly opencode slash-commands under `dot_config/opencode/commands/`; as skills they are read by Claude Code + Codex (via dotagents) and auto-discovered natively by opencode from `~/.agents/skills/<name>/SKILL.md`, so `agents` deliberately stays `["claude", "codex"]` — adding `opencode` would make dotagents write MCP config into the repo-managed readonly `~/.config/opencode/opencode.json`. The now-orphaned deployed `~/.config/opencode/commands` directory is cleaned up by the root `.chezmoiremove`. The MCP servers (`codegraph`, `glab`, `context7`, and Exa `websearch`) AND the remote skills are **data-driven from `.chezmoidata/agents.yaml`** (see Single source of truth): `agents.toml.tmpl` ranges over `.agents.mcp.servers` and `.agents.skills.remote`, resolving header op-refs through the shared recursive `op://` rule at render. Add or remove a server or remote skill in the data file and all consumers pick it up — no more parallel edits.
- Provisioning runs via `.chezmoiscripts/70-agents/run_onchange_after_install-dotagents-skills.sh.tmpl` (linux + darwin gate): it `mkdir -p`s the agent config dirs (`~/.claude`, `~/.codex`) first, then runs `dotagents --user install` through `mise exec npm:@sentry/dotagents@latest`, soft-skips on missing mise/git or install failure, and is kept in containers. Its onchange trigger has **three** parts, because the remote skills track upstream releases and `agents.yaml` also carries unrelated pi/OpenCode data. (1) It fingerprints the raw `dot_agents/private_readonly_agents.toml.tmpl` source for target-format and local-skill changes. (2) A deterministic comment-only hash covers only the unresolved `.agents.mcp` + `.agents.skills.remote` data, so an MCP/remote-skill edit re-triggers the install without making pi/OpenCode-only edits re-run dotagents; hashing unresolved `op://` refs (never rendered targets) keeps secrets out of state. A **rotated 1Password MCP-header secret does not re-trigger the install by itself** — re-provision by hand or with `chezmoi apply --force`. (3) Those raw inputs are **byte-identical across an upstream release** (the refs are template lookups, not literals), so the script ALSO renders **the resolved refs themselves** into a comment block. Without them a new release would land in the rendered `~/.agents/agents.toml` while the script that actually fetches the skill never re-ran. Refs are tags, not secrets, so embedding them is safe. **Caveat:** `shadcn/improve` pins a BRANCH (it publishes no releases/tags) and a branch head is not resolvable by a chezmoi template function, so new commits there do not re-trigger the script — they are picked up whenever the install does run (a config edit, another skill's release, or `chezmoi apply --force`). It was previously the canonical bare-`run_after_` retry exception (every-apply execution + an internal stamp/refresh guard), which existed to let the then-unpinned remote skills advance past `minimum_release_age` and to retry soft-skipped first applies; the age gate is gone entirely now, and the retry need is accepted as the standard onchange trade-off (a soft-skipped/failed install — mise/git/network absent — is recorded as done and retries only on the next fingerprint change or `chezmoi apply --force`). The pre-`mkdir` is the **first-apply fix**: `dotagents install` symlinks `skills` into each agent's config dir (`ensureSkillsSymlink`) without creating the parent, and on a fresh machine `~/.codex` existed (codex external) but `~/.claude` did not until Claude Code was first run — so the symlink `ENOENT`ed, the whole install exited non-zero, and the soft-skip silently swallowed it, leaving skills/MCPs unconfigured until a later apply. Pre-creating the dirs makes the initial apply provision fully. Since the shared-instruction wrappers under `dot_claude/`/`dot_codex/` became chezmoi-managed (see Single source of truth), the file phase itself now creates both dirs before this after-phase script runs — the pre-`mkdir` stays as a belt-and-braces guard.
- **There is no pin-bump workflow and no settle window.** The former `.github/workflows/update-agent-skills.yml` (weekly PR bumping `@`-pinned refs past a 7-day cutoff) and `minimum_release_age = 10080` were both removed: refs now resolve to the newest upstream release on every render, so there is nothing to bump and an age gate would only reject a fresh release and fail the apply. Skills are therefore **unsettled by design** — a bad upstream release reaches the host on the next apply. The `dotagents` CLI itself is unaffected and still observes the standard 24h mise cooldown.
...
- The package is **unpinned** (`npm:pi-mcp-extension` tracks npm latest), the same trade-off the dotagents remote skills make by tracking their newest upstream release: **unsettled by design** — a bad upstream publish reaches the host on the next install.
...
- **All agent configuration**: `.chezmoidata/agents.yaml`. This is the single
  source of truth for MCP servers, dotagents remote skills, pi packages/defaults/
  static auth, OpenCode models/providers, and oh-my-openagent model mappings.
  Change agent data HERE, not in a consuming template.
  ...
  - `agents.skills.remote` drives dotagents. An absent `ref` resolves the newest
    release; a present one is literal (for example improve's `main`). The twelve
    local path skills remain literal in agents.toml because they have one
    consumer.
  ...
  deterministic hash of only unresolved `agents.mcp`/`agents.skills.remote`
  data, keeping secrets out of state without coupling pi/OpenCode-only edits to
  dotagents. A rotated secret follows the consuming script/template's own
  documented onchange behavior.
