...
- **apt packages / repos (Ubuntu)**: `.chezmoidata/packages.yaml` under `packages.linux.ubuntu.*`. The `run_onchange_before_ubuntu.sh.tmpl` renders and installs from it (NVIDIA packages from CUDA + libnvidia-container repos under the `HAS_NVIDIA` gate, the Intel graphics stack + `intelHweKernel` from the kobuk-team/intel-graphics PPA under the `HAS_INTEL_GPU` gate, bare-metal, flatpaks, dotnet tools, and direct `.deb`s). Add packages/items there; editing the data re-triggers the installer.
  - **Mesa (`kisak-mesa` PPA)**: the `kisak-mesa` entry in `aptRepos` adds ppa:kisak/kisak-mesa (updated Mesa, resolute). `upgrade_kisak_mesa()` then upgrades only the installed packages that PPA supersedes — it filters a `dist-upgrade` simulation for Inst lines carrying the `LP-PPA-kisak-kisak-mesa` origin and runs `apt install --only-upgrade` on them, so nothing outside Mesa is touched (the provisioner never does a blanket upgrade). Not GPU-gated (Mesa is desktop-wide).
