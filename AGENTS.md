...
**`pi-mcp-extension` is an MCP *client*, and it is INERT without `~/.pi/agent/mcp.json`** — installing it buys nothing on its own. That config is `dot_pi/agent/private_readonly_mcp.json.tmpl` (private 0600 + readonly 0444, like its sibling `private_readonly_auth.json.tmpl`, because its headers carry `onepasswordRead` secrets — never a literal). It declares the **same three MCP servers** the other agents get from `dot_agents/private_readonly_agents.toml.tmpl` — `codegraph` (stdio), `context7`, `websearch` (both streamable-http) — so pi reaches parity with Claude Code / Codex. The two files are parallel sources for the same set, not one generated from the other: **add an MCP server to both** or pi silently falls behind.
