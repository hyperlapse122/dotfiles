...
Current Linux root-owned config includes NetworkManager Wi-Fi power-save and unmanaged-device drop-ins, keyd defaults, a libinput local override, `locale.conf`, Plymouth config, a Logitech receiver udev rule, IPv4/IPv6 forwarding via `sysctl.d/`, a `modprobe.d/` drop-in (`audio-hda-no-powersave.conf`) that disables Intel HD-Audio power saving (`snd_hda_intel power_save=0 power_save_controller=N`) so HDMI/DisplayPort audio power transitions stop forcing i915 to retrain the DP link and blank the external monitor, a VM-only `sudoers.d/` drop-in granting `%wheel` password-less sudo, and a `docker-prune.service` / `docker-prune.timer` pair under `systemd/system/` that runs `docker system prune --force` + `docker volume prune --force` weekly. All install at mode `0644` except the `sudoers.d/` drop-in (mode `0440`, VM-only) — see `scripts/linux/install-linux-system-config.sh`.
...
**Inside Solaar rules, use the [`mxm4-haptic`](crates/mxm4-haptic/) client** — `Execute: [mxm4-haptic, "<WAVEFORM>"]`. The [`mxm4-haptic`](crates/mxm4-haptic/) crate builds **three** zero-dependency Rust binaries into `~/.local/bin/` from a single `cargo install` during bootstrap:

- **`mxm4-hapticd`** — a `systemd --user` daemon ([`mxm4-hapticd.service`](home/.config/systemd/user/mxm4-hapticd.service)) that is the **sole owner** of the Bolt receiver's HID++ session. It discovers the MX Master 4 and its HAPTIC feature index **natively over hidraw** — enabling receiver `0x41` connection notifications, re-announcing to learn which slots are connected, then sending Root `GetFeature(0x19B0)` to each connected slot; the slot that answers with a feature index instead of `ERR_INVALID_FEATURE_INDEX` is the mouse. **No `solaar show`, no on-disk cache** — device state lives only in memory and is re-discovered on reconnect (it watches `0x41` for disconnect/reconnect, including a different receiver slot). It serializes playback with debounce + per-pulse pacing and writes the HID++ Long play report (feature `0x19B0`, function 4, ~1-2 ms, no ack wait).
- **`mxm4-haptic`** — the thin one-shot client spawned by Solaar rules; validates the waveform name and forwards it to the daemon over the AF_UNIX socket `$XDG_RUNTIME_DIR/mxm4-haptic.sock`, then exits. No device access and no fallback — the daemon is the only writer, so if it is down the pulse is simply skipped.
- **`mxm4-haptic-notify`** — a `systemd --user` notification bridge ([`mxm4-haptic-notify.service`](home/.config/systemd/user/mxm4-haptic-notify.service)) that eavesdrops `org.freedesktop.Notifications.Notify` via `dbus-monitor` and forwards an urgency-mapped waveform to the daemon (critical → `SHARP COLLISION`, normal → `SUBTLE COLLISION`, low ignored; tunable via the `MXM4_NOTIFY_*` env vars in the unit).
...
There is **no `~/.cache/mxm4-haptic.json`** any more — discovery is native and in-memory. The HID++ discovery wire format (Root `GetFeature`, the `0x41` connection notification, the receiver register writes that enable notifications / re-announce devices, and the request `sw_id 0x0E` chosen to avoid colliding with Solaar's fixed `0x0B`) is documented in [`crates/mxm4-haptic/src/lib.rs`](crates/mxm4-haptic/src/lib.rs) and the daemon source, verified against libratbag and Solaar. Coexistence with the running Solaar autostart session is safe: the daemon reads the same broadcast hidraw reports and its receiver-register writes are idempotent.
