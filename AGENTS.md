...
| `.chezmoidata/releases.json` | external tool versions plus per-platform asset URL and sha256; generated by `packages/release-lock`, never hand-edited |
...
External release resolution is moving out of render time into the generated `.chezmoidata/releases.json` lock so a source-state read performs no network I/O; the plan is [`docs/plans/2026-07-27-001-refactor-static-release-artifact-lock-plan.md`](docs/plans/2026-07-27-001-refactor-static-release-artifact-lock-plan.md). The lock is machine-generated by `packages/release-lock` and MUST NOT be hand-edited — add a tool to that package's registry instead; `.github/workflows/refresh-release-lock.yml` re-resolves it hourly and commits to the default branch only when an entry actually changed. A source that fails to resolve is omitted from the resolution and keeps its last committed entry, and the run still fails. Per-platform URLs and checksums come from the GitHub release API's per-asset `digest`, so the refresh never downloads an artifact to hash it. A declared target whose asset the release does not publish is a hard error, never a silent skip; that strictness is what caught `buf` naming its linux arm64 build `aarch64` while darwin and windows use `arm64`. A target upstream genuinely does not build is declared in `emulatedPlatforms`, served by the amd64 artifact under emulation, and marked `emulated: true` in the lock — so an `x86_64` URL under an `arm64` key is deliberate, not a bug. **Migration is incomplete**: the lock currently covers only GitHub-release tools, and the externals, shared templates, and scripts still resolve at render time until their units land, so the lock is not yet authoritative.
