...
- `packages/` is a Bun workspace built on apply with **Vite+** (`vp`). `run_onchange_after_build-opencode-plugins` builds `@h82/opencode-playwright-cli-session-injection` (symlinked to `playwright-cli-session-injection.js` on Linux and macOS), `@h82/opencode-scratch-guard` (symlinked to `scratch-guard.js` on Linux and macOS; enforces the temp-file policy via `$TMPDIR` injection + `/tmp`,`/var/tmp`,`/dev/shm` deny), and `@h82/opencode-mxm4-haptic` (symlinked to `mxm4-haptic.js` on Linux). `@h82/mxm4-haptic` is a library, not a plugin. The separate `run_onchange_after_build-figma-auth` script compiles `@h82/figma-auth` into the regular executable `~/.local/bin/figma-auth`; apply never invokes OAuth. The user runs `figma-auth <opencode|pi>` on demand, which performs fresh DCR/PKCE and atomically writes only the selected harness's private native auth file (`~/.local/share/opencode/mcp-auth.json` or pi's hashed file under `~/.pi/agent/mcp-auth/`). A soft-skipped build preserves the installed executable and retries only after a fingerprint change or `chezmoi apply --force`; it never creates an OpenCode plugin link.
...
| `70-agents/` | agent provisioning, late so it lands on a fully provisioned host (see next section): dotagents MCPs, the single generic Claude Code + Codex plugin installer (`install-agent-plugins`, data-driven from `agents.claude.plugins` / `agents.codex.plugins` + `agents.marketplaces` — it registers compound-engineering with both agents and mxm4-haptic with Claude, each at the path its marketplace resolves to, gating each row by OS + container at render time; OpenCode reads the compound-engineering path directly from its config and pi installs it as a native `git:` source from its managed settings), the Claude settings-value merge (`config-claude-settings`, read-merge-writes `effortLevel`/`model`/`skipDangerousModePermissionPrompt` into the live `settings.json`), plus pi's extensions (`update-pi-extensions`, a single `pi update --extensions`) |
...
| `compound-engineering-ref.tmpl` | newest `compound-engineering-*` release tag (the plugin repo interleaves other tag trains, so plain `gitHubLatestRelease` is wrong); shared by the consumers of that plugin — `.chezmoiexternals/ai-agents.toml` (archive URL + versioned target path), `opencode-plugins-json.tmpl` (opencode.json's `plugin` array reference), `00-tools/run_onchange_after_compound-engineering` (sibling-version prune), and `70-agents/run_onchange_after_install-agent-plugins` (the generic runner's Claude/Codex marketplace registration at the versioned path) — so OpenCode, Claude, and Codex load the SAME extracted release tree (pi diverges to a `git:` source) |
...
### CLIProxyAPI — authenticated loopback infrastructure

`router-for-me/CLIProxyAPI` is managed on Linux and macOS workstation hosts as
a loopback infrastructure service, but no managed agent routes requests through
it. Pi uses its built-in `anthropic` provider directly; the
`@gotgenes/pi-anthropic-auth` package preserves Pi's native `/login anthropic`
flow while adding Claude Pro/Max OAuth compatibility. The explicit empty
`.chezmoidata/agents.yaml` `agents.pi.models.providers` map renders into
`~/.pi/agent/models.json` to clear the former localhost override. OpenCode,
Claude Code, Codex, and Pi otherwise keep direct provider configuration.
...
- `.chezmoiscripts/90-services/run_after_cli-proxy-api-service.sh.tmpl`
  ...
  repeats the shared semantic probe. No managed agent consumes the loopback HTTP
  endpoint; the stable binary link remains for observability and manual
...
panel's background auto-update off, and keeps debug/pprof/file logging off. No
managed agent routes through the service, so it remains a management and
provider-auth substrate rather than an agent request path. The Management secret
(filled into the runtime config from the 1Password reference
...
credential, real client key, generated auth state, loopback agent route, or
compatibility token belongs in source or rendered configuration. Managed agent
configs intentionally contain no CLIProxyAPI endpoint. Provider auth persists
...
- **Claude Code / Codex** — the generic `.chezmoiscripts/70-agents/run_onchange_after_install-agent-plugins.sh.tmpl` (linux + darwin gate, container-KEPT since these are agent skills wanted in a devbox too) registers the same versioned path with each: `claude plugin marketplace add <path>` + `plugin install`, `codex plugin marketplace add <path>` + `plugin add`. It ranges over `agents.claude.plugins` / `agents.codex.plugins` and resolves the `compound-engineering-plugin` marketplace's `localArchive` path from the `agents.opencode.plugins` CE entry (`externalPath`) + `compound-engineering-ref.tmpl` — no version or path literal is copied into the per-agent entries. Both CLIs accept a local directory as a marketplace source, so no git clone happens at CLI-execution time.
...
4. Re-triggers `70-agents/run_onchange_after_install-agent-plugins`, which re-registers the new path with Claude/Codex. Because the CLIs key by the marketplace NAME (from `.claude-plugin/marketplace.json` → `compound-engineering-plugin`), a bumped path is a NEW registration; the generic runner removes the stale registration first (`marketplace remove <name>`, issued only for a path-changing `localArchive` marketplace), then adds the new one. (pi is separate — its `git:` source plus the `00-tools` sibling-version pruner cover it; the generic runner never touches pi.)
...
- The deployed plugin **tree** is gated **Linux-only** (root `.chezmoiignore`) and skipped in containers, since mxm4-haptic drives the Linux-only MX Master 4 haptic daemon. The generic installer itself deploys on linux+darwin and is deliberately KEPT in containers (for compound-engineering); the mxm4-haptic **row** is instead filtered out at RENDER time on darwin and in a container by its marketplace's `os: [linux]` / `container: skip` (KTD3), so there is no whole-script `.chezmoiignore` skip for it any more.
...
- **Claude Code + pi haptic event maps**: `.chezmoidata/haptic.yaml` (same file,
  ...
  `install-agent-plugins` fingerprint hashes BOTH the plugin source tree AND
...
- **All agent configuration**: `.chezmoidata/agents.yaml`. This is the single
  ...
  externals, the Claude Code / Codex plugin lists + marketplace registry, the
  pinned Claude Code settings values, pi packages/defaults/static auth, OpenCode
  models/providers, and oh-my-openagent model mappings. Change agent data HERE,
  not in a consuming template.
  - `agents.pi.models` holds the explicit empty provider map rendered into
    ...
    outside Linux/macOS workstations and in real containers. The 0444 target
  ...
  - `agents.mcp.servers` is transport-neutral (`stdio` → `command`/`args`; `http`
    → `url` + optional `headers`; optional `auth: oauth`) and is ranged over by
    the THREE agent-config templates. The OAuth marker delegates credentials to
    each harness's private native store; credentials and refresh tokens never
    enter source:
    ...
    opencode local/remote type names). Dotagents and OpenCode infer OAuth from
    the headerless remote endpoint; pi additionally receives the explicit
    `"auth": {"type": "oauth"}` its extension requires. The `figma` entry uses
    this form; `figma-auth <opencode|pi>` performs initial authorization for
    those two harnesses, after which their MCP clients own token refresh.
  ...
  - `agents.marketplaces` + `agents.claude.plugins` + `agents.codex.plugins` are
    the single source for Claude Code / Codex plugins, consumed by the generic
    `.chezmoiscripts/70-agents/run_onchange_after_install-agent-plugins.sh.tmpl`.
    The per-agent `plugins` lists are `{name, marketplace}`; `marketplaces` is the
  ...
  - `agents.pi.settings` is the whole pi settings.json object (renamed from
    ...
    `resolve-op-refs-json.tmpl`). `packages` is nested inside it and ALSO ranges over in the
    update-pi-extensions script, where each entry is resolved to its latest
    version/reference and baked into comment lines as the onchange trigger (the
  ...
  - `agents.opencode.plugins` is the single plugin manifest for both
    ...
    (the generic runner) registers the same path with Claude Code and Codex,
    resolving the `localArchive` path from this CE entry's `externalPath` — every
    consumer reads the same version segment (see the compound-engineering
    section; pi installs CE separately as a `git:` source).
...
- **Repo-root `mise.toml`** is the dev toolchain for working on this repo itself:
  bun + rust, the `aube` package manager used by mise's npm backend, and the
  npm-backed CodeGraph CLI. Its `postinstall` hooks run the package-workspace
  install and Rust fetch through
  explicit bash wrappers, then initialize this checkout's `.codegraph/` index.
  It is chezmoiignored and trusted once via
  `.chezmoiscripts/00-tools/run_once_before_mise-trust.sh.tmpl`; `mise.lock`
  records the resolved tool versions and checksums. There is no repo-scoped
  `agents.toml`; agent skills and MCPs are provisioned only through the
  user-scoped, chezmoi-rendered configuration described above.
...
- **Only the CLI dotfiles deploy in a container.** The `.chezmoiignore` container
  ...
  and reconciler are excluded because it is host-local infrastructure, not a
  container CLI. The empty Pi models baseline is also excluded because it exists
  only to migrate workstation hosts away from the former route. Adjust skips in
  that one gated block, never by editing the scripts.
