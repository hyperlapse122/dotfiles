...
#   ce-overlay-lock-hold.sh resolve-latest [--lock FILE] [--update]
#
...
#   --update               resolve-latest only: write the resolved entry into
#                          --lock instead of a scratch copy
#
# STEPS
...
#     any other status from the gate or the driver
#                  roll the stamp back and restore; no candidate, class
#                  tooling-failure. The refresh of every other tool still commits,
#                  and the workflow fails after the push.
#
# RESOLVE-LATEST. Resolves only the compound-engineering entry and prints its
# version on stdout. Without --update the resolver writes into a scratch copy of
# --lock. Exit 2 when the resolver fails. The workflows share this one entry
# point, so the resolver flags and the lock key live in one place.
#
...
# a failure to read or restore the lock, 64 a usage error or a missing tool.
...
while [[ $# -gt 0 ]]; do
  case $1 in
    --update)
      [[ $mode == resolve-latest ]] || usage_error "unknown argument: $1"
      update=true
      shift
      ;;
    --lock | --committed-lock | --overlay-dir | --pristine-dir)
      if [[ $mode == resolve-latest && $1 != --lock ]]; then
        usage_error "resolve-latest does not take $1"
      fi
...
if [[ $mode == resolve-latest ]]; then
  target_lock=$lock
  if [[ $update != true ]]; then
    target_lock="$scratch/latest-lock.json"
    cp -- "$lock" "$target_lock"
  fi
  resolve_bun
  [[ -n $BUN_BIN ]] || die 'bun is required to resolve the latest release'
  "$BUN_BIN" "$repo_root/packages/release-lock/src/cli.ts" --only "$TOOL" --out "$target_lock" >&2 || exit 2
  jq -r --arg tool "$TOOL" '.releases.tools[$tool].version // ""' "$target_lock"
  exit 0
fi
...
case $rc in
  ...
  *) hold no tooling-failure "The overlay gate failed with status $rc, so the release was not validated. No rebase is dispatched." ;;
esac
...
case $rc in
  ...
  *)
    rollback
    hold no tooling-failure "The overlay driver failed with status $rc, so base.json was not stamped. No rebase is dispatched."
    ;;
esac
...
case $rc in
  ...
  *) hold no tooling-failure "The overlay gate failed with status $rc in pinned mode, so the stamp was rolled back. No rebase is dispatched." ;;
esac
