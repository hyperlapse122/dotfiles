#!/usr/bin/env bash
# Focused gate for U5: the frozen CI-only site matrix, the versioned runtime
...
if registry_lines[0] != 'capability-registry-v1':
    problems.append('registry must start with capability-registry-v1')
if registry_lines[-1] != '':
...
for number, line in enumerate(registry_lines[1:-1], start=2):
    fields = line.split('\t')
    if len(fields) != 5:
        problems.append(f'registry line {number} has {len(fields)} columns, expected 5')
        continue
    key, kind, side_effect, available, unavailable = fields
...
# The registry is the ONLY runtime source of probe keys, so the matrix's
# de-duplicated blocking keys and the registry key set must be the same set: an
# undeclared site-local key on one side or a dead probe on the other is a defect.
...
# The consumer renders three probe reads plus the production fingerprint values
# interface, so one command exercises both token reading and fingerprint wiring.
...
exit 0
STUB
chmod +x "$stub_bin/sudo" "$stub_bin/zsh"
...
# --- a non-Linux host publishes fixed unavailable tokens, unprobed -----------
# Every registry key is a Linux-only condition, so a darwin/windows host must
# publish the registry's fixed unavailable token WITHOUT launching a probe.
...
: >"$log"
env -u DBUS_SESSION_BUS_ADDRESS HOME="$fixture_home" XDG_CACHE_HOME="$cache_home" \
  XDG_RUNTIME_DIR="$runtime_dir" PATH="$darwin_bin:$stub_bin:$tool_bin" FIXTURE_LOG="$log" \
  FIXTURE_SUDO=available CHEZMOI_SOURCE_DIR="$source_dir" \
  bash -c '_INSTALL_PREREQUISITES_TEST_SOURCE=1
    source "$1"
    unset _INSTALL_PREREQUISITES_TEST_SOURCE
    write_capability_cache' bash "$repo_root/$hook" \
  >"$scratch/darwin.out" 2>"$scratch/darwin.err" \
  || fail "a non-Linux host failed to publish a record: $(cat "$scratch/darwin.err")"
darwin_records=("$capability_dir"/*.tsv)
[[ ${#darwin_records[@]} -eq 1 ]] || fail 'a non-Linux host published no single record'
if tail -n +2 "${darwin_records[0]}" | grep -qv $'\tunavailable$'; then
  fail 'a non-Linux host published a token other than the registry fixed unavailable'
fi
[[ "$(count_log 'sudo ')" -eq 0 ]] || fail 'a non-Linux host launched a Linux probe'
[[ "$(count_log 'zsh ')" -eq 0 ]] || fail 'a non-Linux host launched a Linux probe'

# --- once per command, and nothing probes from a template ---
reset_cache
apply_fixture available present
[[ "$(probe_line)" == 'zsh=available sudo=available bus=available' ]] \
...
apply_fixture available present
[[ "$(rendered)" == "$stable_first" ]] || fail 'two commands with the same host state rendered different content'
flip_reference=$(rendered)
mv -- "$stub_bin/zsh" "$scratch/zsh.hidden"
reset_cache
apply_fixture available present
[[ "$(probe_line)" == 'zsh=unavailable sudo=available bus=available' ]] \
...
# --- session-bus stays an environment/socket test; sudo stays fail-closed ---
reset_cache
apply_fixture unavailable absent
[[ "$(probe_line)" == 'zsh=available sudo=unavailable bus=unavailable' ]] \
...
apply_fixture unavailable absent
[[ "$(probe_line)" == 'zsh=available sudo=unavailable bus=available' ]] \
...
[[ "$(probe_line)" == 'zsh=unavailable sudo=unavailable bus=unavailable' ]] \
