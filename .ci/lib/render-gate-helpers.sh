...
# Shared render-gate helpers for the omp-plugin gate tests
# (.ci/test-unmanaged-repo-guard-gates.sh, .ci/test-mxm4-haptic-gates.sh).
#
# Sourced only, never executed directly. Every helper below takes
# `repo_root`, `scratch`, and `chezmoi_bin` as its first three positional
# parameters, ahead of its own arguments, so both callers share one calling
# shape even where a given helper reads none of the three. `fail` is left to
# the sourcing script — its message prefix is per-test identity — and is
# called unqualified here, resolving via bash's normal dynamic function
# lookup against whichever test sourced this file.
