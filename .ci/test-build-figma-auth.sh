...
prepare_case() {
  ...
  mkdir -p "$source_dir/packages/figma-auth/dist" \
    "$home_dir/.local/share/chezmoi-commands/incomplete/figma-auth" "$fake_bin"
...
case "${FIGMA_MISE_MODE:-success}:$*" in
  dependency-install:*'vp install --frozen-lockfile'*) exit 41 ;;
...
printf old-executable >"$target"
chmod 0755 "$target"
assert_fatal dependency-install 'build-figma-auth: dependency installation failed'
...
if env HOME="$home_dir" PATH="$fake_bin" /usr/bin/bash "$case_dir/build.sh" \
  >"$case_dir/stdout" 2>"$case_dir/stderr"
then exit 1; fi
grep -F 'build-figma-auth: mise is not installed; figma-auth build cannot run' \
  "$case_dir/stderr" >/dev/null
[[ $(cat "$target") == old-executable ]]

prepare_case directory
mkdir "$target"
if env HOME="$home_dir" PATH="$fake_bin:/usr/bin:/bin" bash "$case_dir/build.sh" \
  >"$case_dir/stdout" 2>"$case_dir/stderr"
then exit 1; fi
[[ -d "$target" ]]
grep -F 'the figma-auth install target is not a regular file' "$case_dir/stderr" >/dev/null
