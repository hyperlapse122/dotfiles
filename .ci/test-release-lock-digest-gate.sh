...
accepts() {
  case_name=$1
  out=
  run_gate "$fixtures/$1" || fail 'expected exit 0, got a rejection'
  pass "$2"
}
...
rejects reject-no-digest.json 'toolx linux-amd64' 'an artifact with neither digest is rejected'

# A digest that is not in the recorded shape is not a digest. chezmoi compares
# the literal string, so a near-miss verifies nothing.
...
# inherit it for another platform, another vendor source, or another kind.
