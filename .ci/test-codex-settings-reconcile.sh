...
[[ $(jq -Sc 'del(.mcp_servers) | del(.hooks)' <<<"$declared") == "$(jq -Sc . <<<"$expected_settings")" ]] \
  || fail 'the declared settings leaves do not expand to agents.codex.settings plus the roster-derived pair'
...
# The two declared events, each keyed positionally WITHIN its own event. The count
# is asserted so a dropped record — which disables that hook silently — or an
# undeclared record fails here rather than in a session nobody is watching.
expected_trust_keys=(
