...
staging_omp="$home_dir/.local/share/chezmoi-commands/incomplete/omp"
mkdir -p "$staging_omp"
cat >"$staging_omp/omp" <<'EOF'
#!/usr/bin/env bash
echo "v1-binary"
EOF
chmod 0755 "$staging_omp/omp"
...
"$reconcile_bin" activate-unit --manifest "$scratch/manifest-v1.json" --unit omp --home "$home_dir"

( exec 3< "$home_dir/.local/bin/omp"; exec sleep 30 ) &
proc_pid=$!
...
"$reconcile_bin" activate-unit --manifest "$scratch/manifest-v2.json" --unit omp --home "$home_dir"

"$reconcile_bin" reconcile-all --manifest "$scratch/manifest-v2.json" --home "$home_dir" --prune >/dev/null

[[ -d "$home_dir/.local/lib/commands/store/omp/v1.0.0" ]] || {
...
[[ -d "$home_dir/.local/lib/commands/store/omp/v2.0.0" ]] || {
