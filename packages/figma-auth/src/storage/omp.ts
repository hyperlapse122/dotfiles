...
export function ompCredentialFilename(serverName = FIGMA_SERVER_NAME): string {
...
export class OmpStorage implements StorageAdapter {
  ...
  constructor(options: OmpStorageOptions = {}) {
    this.authDir = options.authDir ?? join(homedir(), ".omp", "agent", "mcp-auth");
  ...
  async commit(session: CompletedSession): Promise<void> {
    ...
    if (directoryStats.isSymbolicLink() || !directoryStats.isDirectory()) {
      throw new Error(`Refusing unsafe omp auth directory: ${this.authDir}`);
    }
    if ((directoryStats.mode & 0o7777) !== 0o700) await chmod(this.authDir, 0o700);

    const path = join(this.authDir, ompCredentialFilename());
...
function validateExisting(source: string): void {
  ...
  try {
    ...
    throw new Error("Existing omp auth file is malformed JSON");
  }
  if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
    throw new Error("Existing omp auth file must contain an object");
  }
}
