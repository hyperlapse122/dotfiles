...
deadline and a two-MiB output limit. It also registers a `tool_call` handler that
calls the orchestration guard to block the `task` subagent tool in an injected
Orca session.
