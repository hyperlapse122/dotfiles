// Flat ESLint config for @h82/opencode-scratch-guard (per-package, as the
...
export default defineConfig(
  ...
  tseslint.configs.recommended,
  {
    ...
    // TSConfigRootDirs" when an editor lints from the workspace root (where every
    // member's `eslint.config.mjs` is visible).
    languageOptions: {
      parserOptions: {
