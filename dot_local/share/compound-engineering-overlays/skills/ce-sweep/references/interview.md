...
2. **Standing approval is consent, captured verbatim.** Section 2's approval question authorizes source-side writes (Slack reactions, GitHub/GitLab labels) on every future run with no per-run confirmation. Record the literal yes/no. A "no" is not a failure — it leaves that source read-only.
...
1. **Source type** — one of:
   ...
   - `gitlab-issues` — a GitLab project's issues
   - `email-experimental` — an email account/folder (experimental; stored in config as `type: email`)
   (You may also inspect `references/sources/*.md` in this skill directory for any additional source types.)
2. **Identity** — depends on the type:
   ...
   - GitLab: the project path as `group/project` (e.g. `gitlab-org/gitlab` or `group/subgroup/project`). Stored as `target`.
   - Email: the account plus a folder/label hint (e.g. `feedback@acme.com / Inbox`). Stored as `target`.

Then assign the source a **short config id** — a stable, lowercase, hyphenated handle the state file and reports use to name this source (e.g. `slack-alpha`, `gh-issues`, `gl-issues`). Suggest one derived from the type, let the user override. Ids must be unique within `feedback_sources`.
...
**Capture per source:** `type` (`slack` | `github-issues` | `gitlab-issues` | `email`), `id` (short handle), `target` (channel ID / `owner/repo` / `group/project` / mailbox hint).
...
- GitLab: a **label** to apply. Default `feedback:ack`.
...
- GitLab: a label. Default `feedback:resolved`.
...
feedback_sources:
  ...
  - { type: gitlab-issues, id: gl-issues, target: group/project, ack_action: "feedback:ack", closeout_action: "feedback:resolved", sensitive: false, approved: true }
