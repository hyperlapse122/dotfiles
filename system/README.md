...
| `etc/udev/rules.d/` | udev rules: NuPhy Gem80 VIA/WebHID and host lighting daemon access, STM32 ROM DFU bootloader access, Logitech receiver wake disable, DualSense touchpad libinput ignore, Sennheiser BTD 600/700 dongle hidraw access |
...
| `run_onchange_after_install-system-34-face-auth.sh.tmpl` | Fedora face authentication (`irCamera` gate): Howdy from its COPR, the IR emitter enabler from the release lock, the camera path in Howdy's config, the captured emitter configuration from `etc/linux-enable-ir-emitter/*`, and a derived `custom/face-auth` authselect profile carrying `with-howdy` in system-auth, withheld when the greeter's stack would gain the module; reports the interactive face enrollment (and an emitter configuration still missing) | face-auth data, the emitter lock entry, `etc/linux-enable-ir-emitter/*`, or its own content changes |
