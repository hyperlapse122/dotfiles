...
# twice (suspend leg, then hibernate leg); only the hibernate leg stops, so a
# plain suspend resume never bounces the services. Stop runs CPAMP-first (the
# plan's shutdown ordering); start runs CPA-first. The pre record is the sole
# source of truth at post: only units actually stopped are restarted, a
# plain-suspend or suspend-leg post has no record and no-ops, and a post after
# a failed hibernate still restarts because the record survives regardless of
# the resume action variable.
#
...
# Stop runs CPAMP-first so the panel is never mid-request against a dying CPA
# (plan decision: CPAMP-before-CPA on every shutdown path). Start runs CPA-first
# so the proxy answers before the panel reconnects.
STOP_UNITS="cpa-manager-plus.service cli-proxy-api.service"
START_UNITS="cli-proxy-api.service cpa-manager-plus.service"
...
# If any tool we need is absent, do nothing rather than break hibernation/resume.
...
# Restart exactly the units recorded at pre (CPA-first), then drop the record.
start_for_user() {
  mach=$1 record=$2
  [ -f "$record" ] || return 0
  for unit in $START_UNITS; do
    grep -qx "$unit" "$record" 2>/dev/null || continue
...
case $phase in
  pre)
    # Only disk-persisting sleep states trigger a stop. Plain suspend is left
    # alone so a RAM-suspended session keeps its running proxy.
    case $state in
    ...
    ;;
  post)
    # Restart solely from the pre record (see header): no record means a
    # plain-suspend or suspend-leg resume, which must not bounce the services.
    ;;
  *) exit 0 ;;
esac
