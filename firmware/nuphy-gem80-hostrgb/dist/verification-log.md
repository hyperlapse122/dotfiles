...
# 개정 2 검증 — 워치독과 SIDE 리전

- 일시: 2026-09-11T00:16Z (플래시), 이후 연속 관측
- 플래시한 펌웨어: `nuphy_gem80_ansi_hostrgb.bin` (74,132 bytes, sha256 `5fa2aa5790b2a84a…`) — `dist/build-info.json`과 일치
- 플래시 전 장치: 개정 1 (프로브 응답으로 확인)
- 백업: `~/.local/share/gem80-backup/QMK_firmware_nuphy_gem80_trimode_ansi_v2.1.5.bin` (순정 v2.1.5)

## 플래시 전 출처 게이트

통과 — fork SHA, make 타깃, 툴체인 다이제스트, 바이너리 sha256, 키맵 3해시, **패치 1해시** 전부 일치. 패치 항목은 이번 개정에서 새로 게이트에 들어왔다.

## 플래시

```
Downloading element to address = 0x08000000, size = 74116
Download done.
File downloaded successfully
Submitting leave request...
Transitioning to dfuMANIFEST state
...
최초 상태 조회에서 `dfuERROR, status(10) = firmware is corrupt`가 났고 dfu-util이 지우고 진행했다. `Esc` 홀드 진입 시 앱이 깔끔하게 넘겨주지 않아 생기는 STM32 DFU의 알려진 특성이며, 1차 플래시 때와 동일하다. 오류가 아니다.

`dfuMANIFEST` 이후 장치가 `0483:df11`로 남아 조작자가 케이블을 재연결했고, 약 65초 뒤 `19f5:3275`로 재열거됐다.
...
LED count: 101
LEDs per packet: 9
side LED first index: 89
```

Goal Capsule의 중단 조건(총 101, 경계 89)을 만족한다. 호스트 도구는 이 값들을 상수가 아니라 프로브 응답에서 읽는다.

## 수용 예시

| AE | 덮는 요구 | 판정 | 근거 |
| --- | --- | --- | --- |
| AE10 | R13 | 충족 | **플래시 전** 실행. 개정 1 장치에 `enter`/`frame` 모두 종료 코드 7로 거절, 모드 패킷 미전송 |
| AE1 | R1, R4 | 충족 | KEYS 10초 마감, 하트비트 없음 → 조작자 확인: 키가 저절로 저장된 효과로 복귀. 이후 하트비트가 종료 코드 9로 거절 |
| AE3 | R2, R3 | 충족 | 6초 마감으로 45초간 하트비트만 전송, 픽셀 쓰기 0회 → 조작자 확인: 색 유지. 거절·통신 오류 없음 |
| AE4 | R5 | 충족 | direct 모드가 아닌 20초 동안 하트비트가 계속 거절되고 장치 정상 |
| AE5 | R6, R7 | 충족 | 두 리전 진입 후 101 LED 프레임 → 조작자 확인: 키 89 + 스트립 5 + **로고 7** 전부 마젠타 |
| AE6 | R8 | 충족 | KEYS만: 키 빨강, 로고·스트립은 펌웨어 파랑 유지. SIDE만: 로고·스트립 초록, **89개 키는 효과 유지** |
| AE11 | R11 | 충족 | SIDE를 호스트가 잡은 동안 caps lock 토글이 사이드 색을 바꾸지 않음. KEYS만 잡았을 때는 caps lock 정상 동작 — 양방향 확인 |
| AE2 | R4 | 미확인 | 저장된 효과를 "꺼짐"으로 만들 수단을 이 세션에서 확보하지 못했다. 무선과 무관한 항목이며 토글 키가 확보되면 단독으로 확인할 수 있다 |
| AE7 | R9 | 미확인 | 아래 참고 |
| AE8 | R9 | 미확인 | 아래 참고 |
| AE9 | R10 | 미확인 | 아래 참고 |

**AE7·AE8·AE9를 미확인으로 남긴 이유는 조작자의 판단이다.** 이 Gem80은 무선으로 쓰지 않기로 했고, 세 항목이 관측하려는 배터리·충전 표시는 그 사용에서만 의미를 갖는다. 즉 R9와 R10의 펌웨어 경로는 패치가 `bat_led_show()` 호출을 그대로 둔 것으로 코드 수준에서만 확인됐고, 실물 관측은 없다. 나중에 무선 사용이 생기면 이 세 항목부터 확인해야 한다.

이 결정은 R9와 R10이 구현되지 않았다는 뜻이 아니라, 관측되지 않았다는 뜻이다.

### 리뷰가 고친 결함의 실물 확인

두 항목은 코드 리뷰가 아니었으면 이 세션에서 드러나지 않았을 것이다.

- **AE1의 거절**: 워치독 발화 후 하트비트가 종료 코드 9로 거절된다. 수정 전 코드는 이 하트비트를 수락하고 성공으로 응답해, 호스트가 아무것도 표시되지 않는 허공에 프레임을 계속 보내면서도 정상이라 믿게 했다.
- **AE6의 SIDE 전용 경로**: SIDE만 잡았을 때 89개 키의 효과가 끊기지 않는다. 수정 전에는 `mask & KEYS`가 0이라는 이유로 매트릭스를 EEPROM에서 리로드해 진행 중이던 효과가 초기 프레임으로 되돌아갔다.

### 부수 관측: 단일 소유자 전제

`hold`가 도는 중에 같은 hidraw 노드로 `frame`을 동시에 실행하자 두 프로세스가 서로의 응답을 가로챘다 — 하트비트가 프로브 응답을(`60 00`), 종료 명령이 set 응답을(`60 02`) 받았다. 도구는 이를 `EXIT_MALFORMED_RESPONSE`(6)로 잡아냈다.

계획이 상정한 대로 HID 장치는 단일 소유자를 전제한다. 데몬 작업에서 이 전제가 설계에 명시되어야 하며, 코드 리뷰가 P2로 지적했던 입력 버퍼 오염이 실물에서 재현된 사례다.
