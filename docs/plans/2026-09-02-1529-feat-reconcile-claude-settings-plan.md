...
- KTD3. **Satisfy concurrency with an atomic rename plus a content re-read immediately before it.** Governs R11, R12. The rename is atomic, so a concurrent reader sees either the whole old file or the whole new one and never a truncated one, which is what R12 requires. The lock file is zero bytes with no documented protocol, so taking it would guess at the vendor's locking discipline and risk stalling every apply. The lost-update direction is not self-healing for the other writers' keys, so it is detected rather than accepted: the reconciler re-reads the target's canonical content immediately before the rename and discards its staged file when it differs from what it read. Content comparison rather than inode plus mtime, because the whole staging window is tens of milliseconds — an in-place rewrite keeps the inode and lands in the same whole second, so an identity check would pass while the write was lost. (Resolves the deferred concurrency question.)
...
- **Test scenarios:**
  ...
  - A declaration whose value is a map or a list fails the render: a declared path must name a leaf.
  - A declaration holding both an ancestor path and a leaf beneath it fails the render, naming both paths.
...
- **Approach:**
  ...
  7. Keep the staged file in the target directory and the `mv` plus `restorecon`, per KTD3 and R11, and re-read the target's content immediately before the rename per KTD3.
...
- **Test scenarios:**
  ...
  - A write that lands between the read and the rename is detected: the staged file is discarded, the other writer's content survives, and the run reports on stderr.
  - An empty target is backed up and rebuilt rather than halting assertion on every later apply.
  - A symlink planted at the backup path is refused, so the backup cannot be written through it into another file.
...
- The staged file is created in the target directory and moved into place by rename, with the target's content re-read immediately before the rename and `restorecon` applied afterwards.
