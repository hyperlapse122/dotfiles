...
- 패치 대상은 두 파일이다: `keyboards/nuphy/gem80/side.c`(사이드·로고 두 효과 단계)와 `keyboards/nuphy/common/core/keyboard.c`(caps lock 사이드 표시 억제, R11). 후자가 `common/` 아래에 있으므로 fork가 gem80 외 보드를 건드리는 커밋으로 움직일 때 충돌 빈도가 KTD6이 상정한 것보다 높을 수 있다.
