...
- **Approach:**
  ...
  5. 스캐너 자체는 손대지 않는다. R13이 요구하는 동작은 이미 그 안에 있다.
...
- **Approach:**
  ...
  3. 같은 이름으로 다시 설치할 때 기존 사본이 덮어써지는지 확인한다(A3). 덮어쓰지 않아 제거 후 설치로 처리해야 한다면, 설치와 활성화가 모두 성공할 때까지 기존 사본을 보존했다가 어느 쪽이든 실패하면 되돌린다. 교체 도중 훅이 사라진 창에서는 fail-open 정책상 감독되지 않는 피어가 그대로 뜬다.
  4. `agy`가 없는 호스트의 건너뛰기를 저장소의 skip 선언 메커니즘(`.chezmoitemplates/skip.sh.tmpl`)으로 transient-tolerable 블록으로 선언한다. 이유를 stderr에 남기고 성공으로 끝나되, skip 레코드가 남아 CLI가 설치되면 다시 수렴한다. 그 밖의 preflight 오류는 계속 크게 실패한다.
...
- **Test scenarios:**
  ...
  - `agy`가 없는 호스트에서 이유를 남기고 성공으로 끝나며, skip 레코드가 남는다.
  - skip 레코드가 남은 뒤 `agy`가 설치되면 다음 적용에서 수렴한다.
  - 교체 중 설치나 활성화가 실패하면 기존 사본이 되살아나고, 게이트가 계속 작동한다.
...
- **Test scenarios:**
  ...
  - 선언이 검증 템플릿을 통과한다.
- **Verification:** `orca-ide orchestration worker-start --agent <확정된 식별자>`가 수락된다. 인스트럭션 게이트와 설정 리콘실러 게이트가 통과한다.
