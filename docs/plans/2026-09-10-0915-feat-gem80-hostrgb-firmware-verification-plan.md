...
- **Objective:** NuPhy Gem80이 호스트가 지정한 색으로 89개 키 LED를 개별 점등하고, 호스트가 물러나면 저장된 효과로 되돌아온다. 이 동작이 실제 장치에서 관찰로 증명된 상태가 목표다.
...
- **Stop conditions:** 플래시 중 장치가 DFU로 열거되지 않거나 `dfu-util`이 오류로 종료하면 멈추고 복구 절차(R5)를 먼저 실행한다. `60 00` 프로브가 89가 아닌 LED 개수를 응답하면 멈추고 보고한다 — 키맵이 잘못된 보드로 빌드된 신호다.
...
Gem80의 순정 펌웨어를 `hostrgb` 커스텀 빌드로 교체하고, `0x60` 벤더 명령이 89개 키 LED를 실제로 칠하는지 장치에서 증명한다. 리포에는 직접 저작한 키맵 3파일과 커밋 SHA로 고정된 빌드 경로만 남는다.

### Problem Frame

Gem80 하드웨어와 펌웨어 내부는 이미 퍼키 단위로 동작한다. `keyboards/nuphy/gem80/config.h`가 `RGB_MATRIX_LED_COUNT`를 89로 정의하고 (`keyboard.json`의 `rgb_matrix` layout 88개를 `undef`한 뒤 재정의한다), fork는 `rgb_matrix_set_color()`를 인덱스별로 호출하는 커스텀 효과를 이미 싣고 있다. 빠진 것은 호스트가 그 89개 색을 공급할 전송 경로 하나뿐이다.
...
- R6. `60 00` 프로브가 프로토콜 리비전, LED 개수 89, 패킷당 LED 9를 응답한다.
- R7. `60 01 01`로 direct 모드에 진입한 뒤 `60 02`가 지정한 키가 지정한 색으로 점등하며, 89 LED 전체 프레임(10패킷)이 반영된다.
...
- SIDE 영역 지원 (로고 7 + 사이드바 5, 총 12 LED)
  - **Still to decide** `0x60`에 영역(`KEYS`/`SIDE`)을 어떻게 실을지. 이 계획이 검증하는 명령에는 영역 바이트가 없다 — R6–R8은 `KEYS` 89개만 다룬다. SIDE를 붙이려면 명령을 확장해야 하고, 그 확장은 이미 플래시된 펌웨어를 다시 굽는 일이다.
...
- 89개 키 LED와 12개 SIDE LED는 별도의 WS2812 체인이며, `0x60`의 `KEYS` 영역은 전자만 다룬다. 하드웨어에서 확인되었다.
...
sequenceDiagram
  ...
  K-->>H: echo
  Note over H,K: 89 LED 전체 프레임 = 10 패킷
...
- **Approach:**
  ...
  2. 하위 명령별 서브커맨드를 둔다: `probe`, `enter`, `exit`, `set <index> <r> <g> <b>`, `frame <r> <g> <b>` (89 LED 전체를 한 색으로).
  3. 각 쓰기는 선행 리포트 ID `0x00` + 32바이트 페이로드다. 응답은 같은 노드에서 타임아웃과 함께 읽는다.
  4. `probe`는 응답을 사람이 읽을 수 있게 풀어 출력한다 — 프로토콜 rev, LED 개수, 패킷당 LED. LED 개수가 89가 아니면 0이 아닌 코드로 종료한다 (Goal Capsule의 stop condition).
...
- **Test scenarios:**
  ...
  - `frame`이 89 LED를 패킷당 최대 9개로 쪼개 정확히 10개 패킷을 만들고, 마지막 패킷의 `count`가 8이다 (9×9=81, 89−81=8).
  - 모든 쓰기 페이로드가 정확히 33바이트다 (리포트 ID 1 + 32).
  - 프로브 응답의 LED 개수가 89가 아니면 0이 아닌 코드로 종료한다.
...
- **Verification:** `verification-log.md`가 7·8·9단계의 결과를 각각 담는다. `probe`가 LED 개수 89를 응답한다. AE1과 AE2가 관찰로 충족된다. 무선 결과가 양성이든 음성이든 두 모드 각각에 대해 기록되어 있다.
