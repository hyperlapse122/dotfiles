...
- **KTD2 — Store only the reference in source data and resolve the secret at apply time.** Add a dedicated CLIProxyAPI data record containing `op://tum6wsa7azjvbkgwnp6fgamcvm/CLI Proxy API/password`, not the value. The after-phase reconciler obtains the existing login password through the official `op` CLI, validates it as a single-line high-entropy secret of at least 32 printable non-space ASCII characters (excluding double quote and backslash so the YAML and curl config scalars remain unambiguous), and keeps it in shell memory only long enough to construct and prove the runtime configuration.
...
- The maintainer will maintain the existing 1Password value at `op://tum6wsa7azjvbkgwnp6fgamcvm/CLI Proxy API/password` as a generated single-line credential with at least 32 printable non-space ASCII characters and no double quote, backslash, newline, carriage return, or control character. A missing or invalid value is a fail-closed runtime condition, not a reason to substitute another secret.
