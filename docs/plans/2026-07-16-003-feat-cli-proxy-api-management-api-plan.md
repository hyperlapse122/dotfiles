...
- R3. A private runtime configuration may carry only the upstream-generated bcrypt form of the credential after bootstrap, must be owner-controlled, and must be locked read-only before the service is considered ready.
...
- AE3. **Read-only configuration.** Given a successful authenticated Management request, when the raw managed configuration and runtime configuration are inspected or a Management config write is attempted, then the source file remains unchanged and the runtime configuration cannot be persisted through the API or by the service process.
...
- Fine-grained Management API authorization beyond the upstream single-key model.
...
- The maintainer will create or maintain the 1Password item `op://Private/CLIProxyAPI/Management API Password`, containing a generated single-line credential with at least 32 printable ASCII characters and no newline, carriage return, or YAML control character. A missing item is a fail-closed runtime condition, not a reason to substitute another secret.
...
- The upstream Management API's config-write endpoints are intentionally unavailable for persistence because the runtime copy is locked read-only. This issue requires authenticated API access and usage reads, not runtime policy editing.
...
- **HTTP surface:** Management routes gain bearer-key authority only on the loopback listener. Provider routes remain credential-free and fail with the existing no-auth response. Panel and plugin resources remain absent and return 404.
...
- **Config write authority:** Upstream Management handlers attempt to write their configured path. The runtime path is locked after bootstrap, so those writes fail closed; tests must prove the managed source and runtime bytes do not change after authenticated requests.
...
- **Approach:** Add a probe-only credential input that is never passed to the service child. Send the authenticated header through a short-lived owner-only file referenced by curl, then remove it before scanning managed state and scratch. Assert upstream's unauthenticated Management failure and successful authenticated `/v0/management/config` response without requesting raw YAML; retain the existing health, provider-routable 503, panel/plugin 404, listener/PID/executable, config immutability, empty auth state, canary, and final handoff checks. Update native smoke to exercise runtime-config bootstrap and lock before the real binary is treated as ready, and assert no credential in process args, supervisor output, request logs, or the isolated home after cleanup.
...
- **Verification:** The full render workflow reaches green, uploaded artifacts contain no credential or request canary, and documentation review finds no statement that the service remains credential-free or that Management API config writes are supported.
...
| Runtime configuration attestation | U1-U3 | Bootstrap briefly permits private conversion, upstream bcrypt is observed, the runtime file becomes owner-read-only, no plaintext remains, source config remains unchanged, and Management config writes cannot persist. |
| Shared semantic probe | U3 | Unauthenticated Management access fails, authenticated local access succeeds, provider-routable `agent` remains HTTP 503 JSON with `no auth available`, optional routes remain 404, and PID/executable/listener identity is re-proven after all checks. |
...
- A private runtime configuration is generated only during apply-time activation, is bcrypt-only and mode 0400 after bootstrap, is not writable through the Management API, and is removed on failed activation or invalid credential input.
...
- The shared probe proves unauthenticated rejection, authenticated loopback success, provider-routable no-auth 503, optional-route 404, runtime immutability, listener ownership, and final PID handoff.
...
- README and AGENTS document the active Management API boundary, apply-time 1Password source, locked runtime state, rotation/failure behavior, and deferred consumers; `CLAUDE.md` remains the exact one-line `@AGENTS.md` pointer.
