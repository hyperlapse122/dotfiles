...
- **Test scenarios:** Covers R5. Every externals file renders with egress blocked. Each tool's rendered URL matches the pre-migration baseline for linux, darwin, and windows. The musl-linux branch still selects the musl asset. A tool with per-OS variants resolves the right asset per platform. Recorded digests appear in the lock for every tool that previously had none and whose upstream publishes one, and the previously-verifying consumers (pi, aoe, jq, and the static-checksum ai-agents tools) keep the same checksum values and remain the only enforcement points.
