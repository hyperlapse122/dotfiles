...
- **Tail ownership:** Local proof is the isolated render matrix over the Linux and container fact shapes plus the grep sweep. The Darwin leg is not locally renderable (see KTD6) and is owned by `render-dotfiles.yml`'s `apply-macos` job on the pull request, which also owns final `ci.yml` proof. The host-local steps in `docs/decommission/ydotool.md` are the operator's; this run writes them and never performs them.
...
- R12. `docs/decommission/ydotool.md` exists and covers what no apply may do: stopping and disabling the user unit, confirming `/dev/uinput` is actually released, unmasking the root `ydotool.service`, and removing the RPM. It states the repository's established apply-first ordering and gives a fallback for a daemon that does not release the device on stop.
...
- AE1. **Covers R1–R7, R15.** Given the edited source, when the isolated harness renders the Fedora KDE, Fedora GNOME, headless Fedora, and container fact shapes, then no rendered target or script contains `ydotool`, the two prune entries appear only under the eligible shapes, and every unrelated rendered target is byte-identical to its pre-change rendering.
...
- AE5. **Covers R12, R13, R15.** Given the branch diff, then it adds no `run_*` script, adds exactly the two gated `.chezmoiremove` entries, and `docs/decommission/ydotool.md` uses the repository's apply-first ordering.
...
- **KTD1a. Ordering is not load-bearing, and the checklist follows the repository's existing apply-first pattern.** An earlier draft of this plan required stop-and-disable *before* the apply, on the theory that deleting the unit file leaves a `graphical-session.target.wants` symlink `systemctl --user disable` can no longer resolve. That theory is false and was disproven directly: `systemctl --root=<scratch> disable` on a unit whose file had been deleted printed `Unit ce-probe.service does not exist` **and still removed the dangling symlink**, exiting non-zero. A running service also stays stoppable by name while systemd holds its loaded unit. Both hazards therefore dissolve, so the checklist matches `docs/decommission/cli-proxy-api.md` and `docs/decommission/open-design.md` — apply first, then stop — instead of diverging from precedent for a non-issue. The one real consequence is cosmetic: between the apply and the disable, a graphical login may log a failure to start a unit that no longer exists. Governs R12.
...
- **KTD6. The local render matrix covers the Linux and container shapes only; Darwin coverage comes from CI.** Rejected alternative: include a macOS fact shape in the isolated `chezmoi execute-template` harness. Reason: `.chezmoitemplates/facts.tmpl` binds the `os` fact to chezmoi's own `.chezmoi.os` builtin, which resolves from the running binary's platform and cannot be overridden by a template variable or CLI flag. The repository already accepts this — `render-dotfiles.yml` dedicates a real `macos-26` runner (`apply-macos`) rather than faking Darwin from Linux. Claiming a local macOS leg would make the Verification Contract unrunnable as written. Governs the Verification Contract and U5.
...
- **Test scenarios:**
  ...
  - Render the headless Fedora and container fact shapes; assert `.chezmoiremove` emits neither prune path, so a host that never received the files cannot have a same-named file deleted.
...
- **Approach:** Mirror `docs/decommission/cli-proxy-api.md`: a label blockquote stating the document is manual operator guidance that chezmoi never executes, then numbered sections in the repository's established apply-first order. Cover: apply the updated source (noting that `.chezmoiremove` reclaims the deployed unit and wrapper, so no manual file deletion is needed); stop the user unit, which is what lets the wrapper's own trap unlink the socket and terminate `ydotoold`; disable it, warning that the command exits non-zero with "unit does not exist" and still removes the dangling `graphical-session.target.wants` symlink, and that a graphical login between apply and disable may log one failed start; confirm `/dev/uinput` is actually released, with a fallback for a daemon that does not exit — identify the surviving process, terminate it directly, and never kill a process that cannot be attributed to this stack; run a user `daemon-reload`; unmask the root `ydotool.service` that `configure_ydotool_package_service` masked; remove the RPM. Note that `/etc/udev/rules.d/80-uinput.rules` is reclaimed automatically by the system installer manifest, mirroring how the existing checklists describe their system-sleep hooks. State that the Solaar `/dev/uinput` grant stays in place and must not be removed. Note per A2 that the runtime socket needs no step of its own.
- **Patterns to follow:** `docs/decommission/cli-proxy-api.md` structure, its apply-first ordering, its "a missing path is fine" idempotence framing, and its "never kill a process you cannot attribute to this stack" caution; `docs/decommission/open-design.md` for the deployed-file enumeration style.
...
- **Approach:**
  ...
  3. Render every changed template and script through the isolated `chezmoi execute-template` harness described in the root `AGENTS.md` (per-user scratch directory, stub `op`, empty config, throwaway destination, `--source "$PWD"`), across the Fedora KDE, Fedora GNOME, headless Fedora, and container fact shapes. Scripts are not targets, so compare them as rendered text on both sides of the change. Do not attempt a macOS shape locally (KTD6) — record that the Darwin leg is delegated to `render-dotfiles.yml`'s `apply-macos` job.
...
- **Test scenarios:**
  ...
  - Covers AE1. Every Linux and container fact shape renders without error, and every unrelated rendered target is byte-identical across the change.
...
- `docs/decommission/ydotool.md` exists, uses apply-first ordering, states the expected non-zero `disable` exit, and carries the stuck-daemon fallback (R12).
...
- The Linux and container render matrix, `git diff --check`, `render-dotfiles.yml` (including `apply-macos`), and `ci.yml` are all green.
...
- `docs/decommission/cli-proxy-api.md` and `docs/decommission/open-design.md` — shape, apply-first ordering, and cautions for U4.
