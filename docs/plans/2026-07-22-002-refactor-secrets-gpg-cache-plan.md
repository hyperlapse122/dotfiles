...
- **The GPG key import is the one always-live-`op` site.** It delivers the key that decrypts the cache, so it cannot read the cache; every other `op://` site — including the Tailscale auth key, now resolved at render time through the shim — uses the cache with a live-`op` fallback. The key import moves to the pre-file phase, and a per-apply probe keeps a readiness marker in sync with the key's actual decrypt-ability.

### Requirements

**Secret cache and resolution**
- R1. A single GPG-encrypted bundle holds every migrated secret value, keyed by its `op://` URI.
- R2. A single fallback shim replaces every `op://` call site except the GPG key import — both the template-time `onepasswordRead` sites and the one former shell `op read` (Tailscale, converted to a render-time shim lookup): it returns the value from the decrypted bundle when GPG is ready and falls back to a live `onepasswordRead` when it is not. On a GPG-ready host, no hot-path render performs a live `op` read.
...
- R12. The GPG key import is the one site that always resolves from live `op` — it delivers the decrypting key, so it cannot read the cache. Every other `op://` site uses the fallback shim, including the Tailscale auth key, which was converted from a shell `op read` in `.chezmoiscripts/10-auth/run_once_after_auth-tailscale.sh.tmpl` to a render-time shim lookup so it too is served from the cache. On a not-yet-ready host these fall back to `op` automatically.
- R13. On a fresh host, the GPG key import is the only guaranteed `op` read; any template site that renders before it on the first apply falls back to `op` once. Every subsequent steady-state apply is `op`-free.
...
- A fresh host's only guaranteed `op` read is the GPG key import, and it is `op`-free on every subsequent steady-state apply.
...
- The `dockerhub` auth script's `onepassword` whole-item fetch is a different mechanism (not an `op://` ref), so it stays live-`op`; it is `run_onchange` and never fires on a steady-state apply, so the op-free steady-state goal still holds.
...
- Exact source path for the committed bundle and the precise `include | decrypt | fromJson` invocation. `.chezmoitemplates/` files are eager-parsed as Go templates; an armored PGP blob has no `{{` so it parses as a literal (safe), but confirm this versus chezmoi's documented root-level `.`-prefixed placement.
...
- **Approach:** `secrets-bundle.tmpl` probes readiness (marker `stat` **and** bundle-file `stat`); when ready it `include`s the bundle ciphertext, `decrypt`s and `fromJson`s it once, and returns the dict; when not ready it returns an explicit empty/not-ready value the shim can detect. Keep the readiness/decrypt logic here so both the shim and the resolver share one implementation.
...
- **Approach:** Extract `op://` refs from the source tree exactly (refs contain spaces — key each entry byte-identically to what the shim looks up), excluding the one ref the shim can never serve — the GPG-key-import ref (never bake the private key into the public bundle). `op read` each remaining value, assemble a JSON object with `jq` (correct escaping), and pipe it over **stdin** into `gpg --encrypt --armor --recipient <fingerprint>`, capturing stdout to `.chezmoitemplates/secrets-bundle.json.asc` — never writing a plaintext bundle file to the source tree. Fail loudly if `op` is unauthenticated or the recipient key is missing, and abort if the private-key ref ever appears in the set. Decoupled from apply; documented as the rotation step. After landing, run it once to materialize the first real bundle.
...
- **Test scenarios:**
  ...
  - The GPG-key-import (private-key) ref is absent from the emitted map, and sync aborts if it ever appears.
  - A space-containing ref (e.g. `op://njbkpy6emfxkbl7n6zmwmz7jfu/WakaTime/API Key`) is keyed byte-identically and, rendered through the shim against this real bundle, produces zero `op` spawns.
...
- The GPG key import runs in the `run_once_before` phase and a per-apply `run_before` probe maintains the decrypt-ability marker; the GPG key import is the only always-live-`op` site.
...
- A fresh-host bootstrap reads `op` only for the GPG key import, plus at most a few pre-key template fallbacks on the first apply, and is `op`-free on subsequent steady-state applies.
