#!/usr/bin/env bash
# scripts/linux/setup-luks-tpm2-unlock.sh
#
...
# ---------------------------------------------------------------------------
# Pure, side-effect-free helpers (unit-tested in isolation; this file is
# sourceable via the main-guard at the bottom so the helpers can be imported).
# ---------------------------------------------------------------------------
...
# Run main only when executed, not when sourced (so unit tests can import the
# pure helpers above without triggering the side-effecting flow).
