...
| `bootstrap/inject-1password-secrets.sh` / `bootstrap/inject-1password-secrets.ps1` | macOS+Linux / Windows | `install.<os>.yaml` `shell:` step (also runnable manually) | Finds every `*.1password` template in the repo, renders it with `op inject`, and writes it under `~/.secrets/<file-name-without-.1password>` when the 1Password CLI is signed in; skips templates when `op` is unavailable or unauthenticated; secret files are mode `0600` on Unix, and secret directories are owner-only/traversable (`0700`) so the files remain readable by the owner |
| `bootstrap/install-fonts.sh` / `bootstrap/install-fonts.ps1` | macOS+Linux / Windows | `install.<os>.yaml` `shell:` step (also runnable manually) | Installs desktop fonts user-wide from GitHub Releases. Uses authenticated `gh` when available and falls back to public release downloads with `curl`; skips already-installed fonts unless `--force` / `-Force`. Add fonts via the registry block at the top of each script |
...
| `linux/install-linux-system-config.sh` | Linux only | `install.linux.yaml` `shell:` step | Recursively discovers files in `system/linux/etc/` and runs `sudo install -D -m 644` to their absolute paths. The `etc/sudoers.d/*` subtree is special-cased: installed at mode `0440` and only on virtual machines (gated on `systemd-detect-virt --vm`); contents are syntax-checked with `visudo -c -f` before install. After the file-install loop, also enables firewalld IPv4 masquerade on the default zone (for Tailscale exit-node and VMware NAT egress paths). The masquerade step is idempotent (`--permanent --query-masquerade` before mutating, `--reload` after) and gated on `firewall-cmd --state` so it skips cleanly when firewalld is not the active backend |
| `linux/install-packages.sh` | Linux only (Fedora + Rocky/RHEL-family) | Manual | Enables distro-appropriate repos plus 1Password, VS Code, Docker, Chrome, and Tailscale repos; installs development tools plus CLI, fcitx5, keyd, .NET, mise, Solaar, 1Password, VS Code, Docker, Chrome, and Tailscale packages where available; adds Steam/Discord only on bare metal; installs selected dotnet global tools; enables available `keyd`, `docker`, and `tailscaled` services; adds the user to available `docker` and `keyd` groups |
