...
- **Windows**: Use `pwsh` (PowerShell) for command execution and examples.
- **macOS/Linux**: Use `zsh` or `bash` for command execution and examples.

## Commit Messages

- If a project defines commit message rules (docs, lint config, commit template, hooks), follow those rules.
- If no project-specific rules exist, use **Conventional Commits**: `type(scope): short summary`.
