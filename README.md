...
- **Model roster:** `.chezmoidata/agents.yaml` defines two lead pins and five worker models by agent and role: Claude lead on `opus[1m]` (Fable is reached through Compound Engineering model elevation, which defaults to the judgment entry's model for whichever of `plan_model` and `brainstorm_model` a repository leaves unset), Codex lead on `gpt-6-astra`, Claude judgment on `fable` at max effort, Claude implementation on `opus` and `sonnet`, Codex judgment and fallback on `gpt-5.6-luna`, and omp implementation and mechanical work on one `google-antigravity/gemini-3.8-flash` seat. Roster changes propagate to settings pins, rendered payloads, and target configs at apply time.
