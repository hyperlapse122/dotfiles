...
Cross-platform dotfiles for **Fedora Linux · Windows · macOS**, kept in one repo and applied with [chezmoi](https://www.chezmoi.io/) via [mise](https://mise.jdx.dev/). chezmoi is never installed globally — mise runs it ephemerally at a pinned version (`2.70.5`).

You bring [`mise`](https://mise.jdx.dev/); the bootstrap brings the rest. One run applies every config into place, installs the mise toolchain, fetches fonts, builds the local Rust/TypeScript helpers, and — on Linux — lays down `/etc` drop-ins, firewall rules, and KDE Plasma preferences. Everything is idempotent: re-run after any `git pull`.

---

## Setup

Pick your OS. Each flow is: **install prerequisites → clone → bootstrap**. The only prerequisite you install by hand is `mise` (plus `git` to clone); mise supplies chezmoi and the Node.js the bootstrap uses.
...
4. **Install the Fedora package set:**
   ...
   This enables RPM Fusion, the keyd/mise COPRs, and third-party repos (1Password, VSCodium, Tailscale, Proton VPN, VirtualBox), installs Chromium and the rootless Podman ecosystem and the rest of the packages, builds the VirtualBox akmods, and enables `keyd` / `tailscaled` / `libvirtd`. **Reboot afterward** — on UEFI + Secure Boot it queues an akmods MOK import, so on the next boot the blue *MOK Manager* screen asks for the one-time password to enroll the signing key and load the freshly-built kernel module; new group memberships also take effect after a re-login/reboot.

5. **Apply the dotfiles and install the toolchain:**

   ```sh
   ./install.sh
   ```

   `install.sh` auto-detects Linux, runs `mise install`, applies every config via chezmoi, builds the local Rust/TS helpers, installs fonts, and writes the root-owned `/etc` drop-ins + KDE Plasma settings (it prompts once for `sudo`). Run it in a real terminal — the `/etc` and KDE steps skip themselves when there is no TTY.

### Windows

1. **Install the prerequisites with winget**, then enable [Developer Mode](https://learn.microsoft.com/en-us/windows/apps/get-started/developer-mode-features-and-debugging) (or run PowerShell as Administrator) so symlinks work:
...
2. **Clone and bootstrap** from PowerShell 7 (`pwsh`):
   ...
   `install.ps1` runs `mise install`, then applies every config via `mise exec chezmoi@2.70.5 -- chezmoi apply`.
...
Run once per OS, `install.sh` / `install.ps1` invoke `mise exec chezmoi@2.70.5 -- chezmoi apply`, which:
...
- **Applies your dotfiles** — chezmoi copies managed files from [`home/`](./home/) (the chezmoi source root) into `$HOME`, handling zsh (Prezto-based), git, SSH, GnuPG, editor settings (VS Code, VSCodium, Zed), and per-OS gating via `.chezmoiignore.tmpl`. Shared Codex settings are *merged* into `~/.codex/config.toml` (never clobbering machine-local per-project trust).
- **Wires live symlinks** — `scripts/bootstrap/link-repo-trees.sh` creates the handful of targets that must stay live symlinks rather than chezmoi-managed copies: cross-tool AI agent rules and slash commands for OpenCode and Codex from [`agents/`](./agents/), the runtime skill tree, and built OpenCode plugins.
- **Builds local helpers** — Rust [`crates/`](./crates/) into `~/.local/bin` (e.g. the MX Master 4 haptic stack — Linux + macOS, autostarted via `systemd --user` / launchd) and the TypeScript [`packages/`](./packages/) Yarn workspace in place.
...
`./install.sh` and `.\install.ps1` are **idempotent** — chezmoi applies only what has changed (managed-copy semantics, not symlinks); `mise install` refreshes tools; helper scripts skip or overwrite deterministic targets safely. Run them again after every `git pull`.
...
| [`.chezmoiroot`](./.chezmoiroot) | Tells chezmoi the source root is `home/` |
| [`home/`](./home/) | chezmoi source root — dotfiles applied to `$HOME`, plus `*.1password` secret templates |
| [`home/.chezmoi.toml.tmpl`](./home/.chezmoi.toml.tmpl) | chezmoi config template (machine-local settings, rendered once on `chezmoi init`) |
| [`home/.chezmoiignore.tmpl`](./home/.chezmoiignore.tmpl) | Per-OS ignore gates (target paths relative to `$HOME`) |
...
| [`codex/`](./codex/) | Tracked shared OpenAI Codex config merged into `~/.codex/config.toml`, plus `hooks.json` |
| [`.github/`](./.github/) | CI for the `packages/` workspace (build/typecheck/test, lint), the `crates/` Rust workspace (`rust.yml`: cargo check/test), and the rest of the repo (`tooling.yml`: shellcheck, PSScriptAnalyzer, actionlint), plus the hourly opencode-plugin bump workflow |
