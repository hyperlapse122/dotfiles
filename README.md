...
- **Linux system config** installs tracked files under [`system/linux/etc/`](./system/linux/etc/) to `/etc/` (`sudoers.d/` only on VMs, at mode `0440`), disables NetworkManager Wi-Fi power saving, configures firewalld for Tailscale & VMware (IPv4 masquerade, `tailscale0` → `trusted` zone, UDP 41641/3478 on `public`), and applies KDE Plasma 6 font, touchpad, panel, Kickoff, clock, IME, and screen-edge preferences — each step skips cleanly when its prerequisites are absent.
