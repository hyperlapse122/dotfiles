...
//!
//! Sole owner of the Bolt receiver's haptic playback session.
//! Discovers the mouse and its HAPTIC feature index natively over HID (no
//! external CLI, no on-disk cache — device state lives only in memory),
...
fn io_loop(device: HidDevice, rx: Receiver<u8>) -> ! {
    ...
    // idempotent receiver-register writes.
