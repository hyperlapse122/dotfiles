...
**Do NOT prefix rules with a per-press `loadEffect`.** An earlier version did exactly that as a defensive self-heal; it was removed. The Overview effect is enabled by default and stays loaded in normal use — the only times it unloaded were stray `toggleEffect` calls during development. Worse, a per-press `loadEffect` does not actually fix the rule: the idempotent state-gating above reads `activeEffects`, which never reports `overview` while the plugin is unloaded, so the gate keeps firing a no-op `invokeShortcut` regardless. The correct fix for a genuinely-disabled effect is to re-enable it in System Settings, not to paper over it per press. The current rules in [`rules.yaml`](rules.yaml) drive KWin shortcuts (`Grid View`, `Show Desktop`, `Window Quick Tile Left/Right`) and KRunner from the thumb-button mouse gestures, with no `loadEffect`. Each of the four DIRECTIONAL gesture rules runs two `Execute` actions — the `invokeShortcut` / DBus call, then `Execute: [mxm4-haptic, "SUBTLE COLLISION"]` for a tactile confirm (the press-with-no-movement KRunner rule stays single-action and silent, since KRunner self-confirms by appearing).
...
Reference implementation (historical): the Haptic / Mouse Gesture Button rules in [`rules.yaml`](rules.yaml) once used this pattern — a `Later` callback chaining `Execute: [touch, ...]` (marker) and `Execute: [mxm4-haptic, "<WAVEFORM>"]` (helper) within a single `KeyIsDown`-gated sub-rule so neither fired on a tap. Those rules have since been converted to directional mouse-gesture rules (the thumb button is set to "Mouse Gestures" divert mode `2`, and rules key on the `MouseGesture` condition), so the stateful marker / `Later` machinery no longer appears in `rules.yaml`. The `mxm4-haptic` helper itself is still used — but now as a plain stateless `Execute` appended to each directional gesture (no marker, no `Later`, no tap-vs-hold gating), since a completed `MouseGesture` notification is itself the discrete event to confirm. Reinstate the marker / `Later` structure described above only if a button needs tap-vs-hold behavior again.
